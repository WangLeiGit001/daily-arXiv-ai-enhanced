...
TL;DR: UI-Venus-1.5是一个端到端的GUI智能体，通过中间训练、在线强化学习和模型融合技术，在GUI自动交互任务上实现了最先进的性能表现


<details>
  <summary>Details</summary>
Motivation: 解决GUI智能体在实现广泛通用性和强任务性能方面的挑战，开发一个适用于真实世界应用的统一GUI智能体

Method: 1) 使用100亿token和30+数据集进行中间训练建立GUI语义基础；2) 在线强化学习与完整轨迹展开，对齐大规模环境中长时程动态导航的训练目标；3) 通过模型融合将领域特定模型（基础、网页、移动）合成统一的检查点

Result: 在ScreenSpot-Pro (69.6%)、VenusBench-GD (75.0%)和AndroidWorld (77.6%)等基准测试中达到新的最先进性能，显著超越先前基线，并在中文移动应用中展示强大的导航能力

Conclusion: UI-Venus-1.5通过技术创新实现了GUI智能体在通用性和性能上的显著提升，为真实世界应用提供了强大的自动化交互解决方案
...
TL;DR: Agent Banana是一个分层代理规划-执行框架，通过上下文折叠和图像层分解技术解决指令式图像编辑中的过度编辑、多轮编辑对象保真度不足和高分辨率处理问题，在4K HDD-Bench基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决专业图像编辑工作流中的三个核心问题：(1)编辑器的过度编辑问题，(2)现有模型多为单轮编辑且多轮编辑会损害对象保真度，(3)现有评估在约1K分辨率下进行，与实际的超高清(如4K)工作流不匹配。

Method: 提出Agent Banana框架，包含两个关键技术：Context Folding(将长交互历史压缩为结构化记忆以实现稳定长时程控制)和Image Layer Decomposition(执行基于层的局部化编辑以保护非目标区域并支持原生分辨率输出)。建立HDD-Bench基准用于严谨评估。

Result: 在HDD-Bench基准上，Agent Banana在多轮一致性和背景保真度方面表现最佳(IC 0.871, SSIM-OM 0.84, LPIPS-OM 0.12)，在指令跟随方面保持竞争力，同时在标准单轮编辑基准上也表现强劲。

Conclusion: 该研究推动了可靠、专业级的代理图像编辑技术及其在实际工作流中的集成，为解决高分辨率多轮图像编辑的关键挑战提供了有效解决方案。
...
TL;DR: 提出了SemanticMoments方法，通过在预训练语义模型特征上计算高阶时间统计量，有效解决现有视频检索方法过度依赖静态外观而忽略运动语义的问题，在SimMotion基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频表示方法过度依赖静态外观和场景上下文，而非运动动态，这种偏差源于训练数据和目标。传统运动中心输入（如光流）缺乏理解高级语义运动所需的基础。

Method: 提出SemanticMoments方法，这是一种无需训练的简单方法，通过计算预训练语义模型特征的时间统计量（特别是高阶矩）来捕捉运动语义信息。

Result: 在SimMotion基准测试（包含合成数据和人工标注的真实数据集）上，SemanticMoments始终优于现有的RGB、光流和文本监督方法，证明语义特征空间中的时间统计量提供了可扩展且感知基础的运动中心视频理解方案。

Conclusion: 语义特征空间中的时间统计量为以运动为中心的视频理解提供了可扩展且感知基础的方法，能够有效解耦运动与外观，解决现有方法的偏差问题。
...
TL;DR: 提出一个可解释的模块化框架，用于从新闻视频中自动检测和提取屏幕上的个人信息，在确定性和可审计性方面优于生成式多模态方法，达到95.8% mAP@0.5的检测性能。


<details>
  <summary>Details</summary>
Motivation: 视频新闻内容激增需要透明可靠的信息提取方法，但图形布局、排版惯例和平台特定设计模式的多样性使得手动索引不切实际。

Method: 构建带标注的新闻图形语料库，设计可解释的模块化提取流水线，在确定性和可审计条件下运行，与生成式多模态方法进行对比评估。

Result: 检测器达到95.8% mAP@0.5，精度79.9%，召回率74.4%，避免幻觉问题，提供完整的处理阶段可追溯性。生成式方法F1分数略高(84.18% vs 77.08%)但缺乏透明度。

Conclusion: 该研究为现代新闻媒体中的混合多模态信息提取建立了方法严谨且可解释的基线，特别适用于需要透明数据溯源的新闻和分析场景。
...
TL;DR: 本研究探索使用卷积神经网络分析低级别管状腺瘤的整张切片图像，以识别预测结直肠癌长期风险的细微组织学特征


<details>
  <summary>Details</summary>
Motivation: 尽管筛查措施广泛实施，但仍有相当一部分最初诊断为低级别腺瘤性息肉的患者会在后期发展为结直肠癌。识别哪些低风险患者具有更高进展风险是个性化监测和预防策略的关键需求

Method: 利用数字病理学和机器学习技术，特别是卷积神经网络（CNNs），对低级别管状腺瘤的整张切片图像进行全面客观分析

Result: 研究旨在确定CNN算法是否能够检测到预测患者长期结直肠癌风险的细微组织学特征

Conclusion: 机器学习方法可能弥补传统组织学评估的不足，为低风险腺瘤患者的风险分层提供更精准的工具
...
TL;DR: 提出基于场景图的零样本条件机制ASQL Conditioner，通过轻量级语言模型生成视觉引导，在推理时优化扩散模型，提升多对象复杂提示的文本-图像对齐能力


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像合成中复杂多对象、属性和空间关系提示的语义保真度和结构一致性难题，现有方法使用预定义布局图限制了组合灵活性和多样性

Method: 采用场景图框架，开发Attribute-Size-Quantity-Location (ASQL) Conditioner，通过轻量级语言模型生成软视觉引导，在推理时进行优化以指导基于扩散的生成

Result: 模型能够在保持文本-图像对齐的同时，支持轻量级、连贯且多样化的图像合成，提高了对复杂提示的组合能力

Conclusion: 场景图基础的零样本条件机制为文本到图像合成提供了更灵活的视觉引导方法，在保持语义准确性的同时增强了生成多样性和结构连贯性
...
TL;DR: 本研究开发了一个轻量级CNN-RNN模型，使用RGB-D摄像头数据在脚触地前250毫秒内预测足底压力中心位置和触地时间，在楼梯上升过渡场景中验证了可行性，模型可在消费级设备上以60FPS运行。
...
Method: 8名受试者在右小腿佩戴RGB-D摄像头和仪器化鞋垫，执行踏上楼梯任务。训练CNN-RNN模型在脚触地前250毫秒窗口内连续预测前-后足底压力中心和触地时间。

Result: 在150、100和50毫秒预测时域下，COP平均绝对误差分别为29.42mm、26.82mm和23.72mm；TOI误差分别为21.14ms、20.08ms和17.73ms。模型可在消费级笔记本或边缘计算设备上以60FPS运行。

Conclusion: 研究表明从视觉数据预测COP和TOI是可行的，轻量级模型在辅助系统的预测控制方面具有重要应用潜力。
...
TL;DR: 论文提出了VLM-UQBench基准，用于评估视觉语言模型中的模态特定和跨模态不确定性，包含600个真实样本和16种扰动方法。研究发现现有UQ方法存在模态特异性、与幻觉关联弱、难以检测细微不确定性等问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的不确定性量化对安全可靠部署至关重要，但需要定位不确定性的来源（图像、文本或模态不对齐）。现有方法缺乏细粒度的模态感知不确定性评估基准。

Method: 构建VLM-UQBench基准，包含来自VizWiz数据集的600个样本，分为清洁、图像不确定、文本不确定和跨模态不确定子集；设计8种视觉、5种文本和3种跨模态扰动方法；提出两个评估指标量化UQ分数对扰动的敏感性和与幻觉的关联性。

Result: 实证发现：(1)现有UQ方法表现出强烈的模态特异性且严重依赖底层VLM；(2)模态特定不确定性常伴随幻觉，但当前UQ分数提供的风险信号弱且不一致；(3)UQ方法在明显群体模糊性上可与思维链基线媲美，但难以检测扰动引入的细微实例级模糊性。

Conclusion: 当前UQ实践与可靠VLM部署所需的细粒度、模态感知不确定性之间存在显著差距，需要开发更有效的不确定性量化方法。
...
TL;DR: IR-SIS：基于自然语言描述的迭代细化手术图像分割系统，结合SAM3、视觉语言模型和智能体工作流，支持临床医生交互式反馈，在领域内外数据上均取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有手术图像分割方法受限于预定义类别、缺乏自适应细化机制和临床医生交互能力，无法满足机器人辅助手术的实时指导需求

Method: 使用微调SAM3进行初始分割，通过视觉语言模型检测器械并评估分割质量，采用智能体工作流自适应选择细化策略，支持自然语言反馈的临床医生交互

Result: 在EndoVis2017和EndoVis2018基准测试中构建多粒度语言标注数据集，实验显示在领域内和分布外数据上均达到最先进性能，临床医生交互带来额外提升

Conclusion: 建立了首个基于语言的手术分割框架，具备自适应自细化能力，为机器人辅助手术提供了更灵活、交互性更强的分割解决方案
...
TL;DR: 论文分析了扩散变换器中基于调制机制的文本条件化是否必要，发现传统用法中池化嵌入贡献有限，但将其用作引导可实现可控属性迁移，带来显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 研究扩散变换器中调制机制文本条件化的必要性，探索其潜在性能优势，特别是在传统注意力机制之外的价值。

Method: 通过分析传统调制机制的效果，提出将池化文本嵌入作为引导信号的新方法，实现训练自由的可控属性迁移。

Result: 发现传统调制贡献有限，但作为引导时能显著提升性能，适用于文本到图像/视频生成和图像编辑等多种任务，且无运行时开销。

Conclusion: 调制机制在传统用法中非必需，但作为引导信号能有效实现可控生成，为扩散模型提供简单高效的条件化方案。
...
TL;DR: X-Mark是一种针对胸部X射线图像的样本特异性清洁标签水印方法，使用条件U-Net在显著区域生成独特扰动，通过多组件训练目标确保水印有效性、鲁棒性和诊断质量保持，并采用拉普拉斯正则化实现尺度不变性。


<details>
  <summary>Details</summary>
Motivation: 高质量医学影像数据集对深度学习训练至关重要，但未经授权使用引发版权和伦理问题。现有针对自然图像的数据集所有权验证方法在医学影像上表现不佳，因为静态水印模式在动态高分辨率扫描中缩放效果差，且医学图像视觉多样性有限、解剖结构微妙，同时需要保持诊断质量。

Method: 使用条件U-Net为每个样本的显著区域生成独特扰动；设计多组件训练目标确保水印有效性、对动态缩放过程的鲁棒性，同时保持诊断质量和视觉可区分性；在训练目标中加入拉普拉斯正则化以惩罚高频扰动，实现水印尺度不变性；在黑盒设置下进行所有权验证以检测可疑模型中的特征行为。

Result: 在CheXpert数据集上的广泛实验验证了X-Mark的有效性，实现了100%的水印成功率（WSR），在Ind-M场景中将假阳性概率降低了12%，并展示了对潜在自适应攻击的抵抗力。

Conclusion: X-Mark为胸部X射线图像提供了一种有效的版权保护解决方案，能够在保持诊断质量的同时实现鲁棒的水印嵌入和验证，解决了医学影像特有的水印挑战。
...
TL;DR: GAFRNet是一个结合图注意力机制和模糊规则的网络，用于乳腺癌病理图像分类，在有限标注下表现优异且具有可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习在医学图像分类中存在标注数据稀缺和模型可解释性差的问题，阻碍临床应用。

Method: 构建相似性驱动的图表示建模样本间关系，使用多头图注意力机制捕获复杂关系特征，同时采用可微分模糊规则模块编码拓扑描述符为可解释的诊断逻辑。

Result: 在BreakHis、Mini-DDSM和ICIAR2018三个基准数据集上，GAFRNet在多种放大倍数和分类任务中均优于现有先进方法。

Conclusion: GAFRNet具有优异的泛化能力和实用价值，可作为弱监督医学图像分析的可靠决策支持工具。
...
TL;DR: 对13种深度学习模型（4种CNN和8种Transformer）在膀胱癌分类任务中的性能评估，包括分类精度、校准分析和可解释性研究，发现ConvNext泛化能力有限，ViT校准效果更好，不同模型适用于不同分布样本。


<details>
  <summary>Details</summary>
Motivation: 由于医学影像中异常区域通常只占图像很小部分，基于ViT和CNN的深度学习模型在自然数据集上的优异性能可能无法直接迁移到医学影像领域，因此需要评估这些模型在膀胱癌分类任务中的表现。

Method: 1) 使用13个模型进行标准分类评估；2) 校准分析检查模型校准效果；3) 使用GradCAM++评估模型可解释性；4) 在公开多中心膀胱癌数据集上进行约300次实验；5) 采用测试时增强提高可解释性。

Result: ConvNext系列在膀胱癌图像分类中表现出有限的泛化能力（约60%准确率）；ViT相比ConvNext和Swin Transformer显示出更好的校准效果；没有模型能提供通用的可解释解决方案。

Conclusion: ConvNext系列适用于分布内样本，而ViT及其变体更适合解释分布外样本，不同模型在医学影像分类任务中各有优劣，需要根据具体应用场景选择合适模型。
...
TL;DR: Kyrtos方法：基于聚类的曲线图表识别与分析系统，通过识别线段中点、提取行为特征，构建属性图并转换为自然语言描述和随机Petri网。


<details>
  <summary>Details</summary>
Motivation: 技术文档中大量图表包含宝贵知识，需要对图表中的曲线等模态进行准确分析以支持深度理解。

Method: 采用聚类方法识别曲线线段的中点边界，解析线段提取方向、趋势等行为特征，构建属性图保持结构特征，并转换为自然语言和随机Petri网。

Result: 通过结构相似性度量验证，在多函数图表上Kyrtos的识别和分析方法表现出高精度。

Conclusion: Kyrtos方法有效实现了技术文档中曲线图表的自动识别与分析，为技术文档的深度理解提供了可靠工具。
...
TL;DR: 本研究系统评估了10种深度学习方法在医学图像领域自适应(DA)中的应用，涵盖多模态数据、噪声处理、联邦学习、可解释性和分类器校准等多个场景。实验表明DA在脑肿瘤数据上能提升4.7%性能，在噪声环境下提升约3%准确率，并能改善模型可解释性和校准性能。


<details>
  <summary>Details</summary>
Motivation: 领域自适应在机器学习中快速发展，但许多方法的核心思想仍是不同域数据在共享特征空间中对齐。本研究旨在探索深度学习模型在医学图像DA中的应用，特别是在多模态、噪声数据、联邦学习等复杂场景下的表现。

Method: 使用10种深度学习模型模拟常见DA技术，在四个医学图像数据集上进行实验。研究考虑了多模态数据、高斯噪声、联邦学习框架、Grad-CAM++可解释性分析和分类器校准等多种情境。

Result: 在脑肿瘤数据集上，DA结合ResNet34使模型性能提升4.7%；在噪声环境下，DA提供约3%的准确率提升；在联邦学习中性能提升有限（约0.3%）；DA改善了模型可解释性并降低了预期校准误差约2%。

Conclusion: 领域自适应技术在医学图像分析中具有显著价值，特别是在处理噪声数据和改善模型校准方面表现突出。然而在联邦学习框架中的效果相对有限，需要进一步研究优化。
...
TL;DR: 提出了一种完全可微的双向协同学习框架DBiSL，通过在线双向跨任务协作整合了监督学习、一致性正则化、伪监督学习和不确定性估计四个关键SSL组件，在医学图像分割中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析中高质量标注数据稀缺且标注成本高，现有双任务协作学习方法仅限于单向交互机制（回归到分割），无法充分利用在线双向跨任务协作的潜在优势。

Method: DBiSL框架实现了完全可微的双向协同学习，能够在线进行分割和回归任务之间的双向交互，整合了监督学习、一致性正则化、伪监督学习和不确定性估计四个SSL核心组件。

Result: 在两个基准数据集上的实验表明该方法达到了最先进的性能表现。

Conclusion: 该工作不仅提供了技术贡献，还为统一SSL框架设计提供了新见解，建立了双任务驱动SSL的新架构基础，并提供了一个适用于更广泛计算机视觉应用的通用多任务学习框架。
...
TL;DR: 对5种最先进的图像到3D重建模型在医学数据上的零样本性能进行基准测试，发现从单切片医学图像重建3D存在深度模糊问题，SAM3D在拓扑相似性方面表现最佳，但所有模型在体素重叠指标上均表现一般，表明需要多视图重建方法。


<details>
  <summary>Details</summary>
Motivation: 体积成像成本高且等待时间长，图像到3D基础模型可通过从2D模态重建3D数据解决此问题。但当前基于自然图像训练的模型是否适用于医学数据尚不清楚。

Method: 使用5种最先进的图像到3D模型（SAM3D、Hunyuan3D-2.1、Direct3D、Hi3DGen、TripoSG）在6个医学数据集和2个自然数据集上进行零样本评估，采用体素重叠指标和点云距离指标。

Result: 所有模型在医学数据集上的体素重叠指标表现一般，表明单切片重建存在深度重建失败模式。SAM3D在全局距离指标上表现出最强的拓扑相似性，而其他模型更容易出现重建过度简化。

Conclusion: 单切片医学重建存在局限性，2D医学数据的平面性质导致深度模糊，需要开发多视图图像到3D重建方法来实现可靠的医学3D推断。
...
TL;DR: K-Sort Eval是一个基于视觉语言模型的高效可靠评估框架，通过后验校正和动态匹配策略解决VLM幻觉偏见问题，用(K+1)自由竞争比较和贝叶斯更新实现与人类偏好一致的高效评估。


<details>
  <summary>Details</summary>
Motivation: 视觉生成模型的快速发展需要更可扩展和人类对齐的评估方法。现有众包竞技场平台成本高、耗时长，而VLM替代方案存在幻觉偏见和静态评估效率低的问题。

Method: 从K-Sort Arena收集人类投票数据构建高质量数据集；使用(K+1)自由竞争比较让新模型与现有模型竞争；提出后验校正方法基于VLM预测与人类监督的一致性自适应校正贝叶斯后验概率；采用动态匹配策略平衡不确定性和多样性最大化每次比较的期望收益。

Result: 大量实验表明K-Sort Eval评估结果与K-Sort Arena一致，通常需要少于90次模型运行，证明了其高效性和可靠性。

Conclusion: K-Sort Eval通过集成后验校正和动态匹配，成功解决了VLM评估中的对齐问题和效率问题，为视觉生成模型提供了可扩展且人类对齐的评估解决方案。
...
TL;DR: LARV是一个无需训练、无需数据的层自适应缩放方法，可集成到任何任务向量合并器中，通过分层缩放策略提升模型合并性能，特别针对视觉变换器中浅层干扰和深层特征稳定性的异质性问题。


<details>
  <summary>Details</summary>
Motivation: 现有任务向量合并方法（如TIES、TSV-M、Iso-C/CTS）对所有层进行近似均匀处理，忽略了大型视觉变换器中存在的强烈分层异质性——浅层对干扰敏感而深层编码稳定的任务特定特征。

Method: LARV为每个任务向量在聚合前分配逐层缩放因子，使用简单的确定性调度自适应抑制浅层干扰并放大深层对齐。该方法计算无需数据的层代理并通过轻量级规则转换为缩放因子，支持分层（两级/三级）缩放和连续映射等多种实现方式。

Result: 在FusionBench基准测试中，LARV持续提升了所有任务向量基线的性能，在8/14/20任务设置下均有改进。例如Iso-C + LARV在ViT-B/32上达到85.9%，ViT-B/16上89.2%，ViT-L/14上92.6%。分层分析和损坏测试表明LARV有效抑制浅层干扰并适度放大深层稳定特征。

Conclusion: LARV将模型合并转变为稳健的层感知过程而非均匀处理，该方法与基础合并器正交且增加成本可忽略，是首个实现任务向量合并中分层缩放的工作。
...
TL;DR: 该论文建立了一个算子理论框架来分析具有块结构参数的非线性逆问题的稳定性和统计集中性。在统一假设下，获得了确定性稳定性不等式、最小二乘失配函数的全局Lipschitz界以及非渐近集中估计，得到了与重建算法无关的高概率参数误差界。以高斯溅射渲染算子为例验证了理论，揭示了稳定性-分辨率权衡关系。


<details>
  <summary>Details</summary>
Motivation: 现代成像和可微分渲染中的高维非线性逆问题需要统一的稳定性分析框架，以理解参数估计的基本限制和算法无关的性能边界。

Method: 采用算子理论方法，结合块状Lipschitz几何、局部可识别性和次高斯噪声的统一假设，建立确定性稳定性不等式和统计集中性理论。

Result: 获得了高概率参数误差界，证明了这些界是前向算子的内在性质，与具体重建算法无关。通过高斯溅射渲染算子的实例验证，揭示了图像分辨率与模型复杂度之比对估计误差的固有约束。

Conclusion: 该分析为现代成像和可微分渲染中的一大类高维非线性逆问题提供了算子层面的基本限制特征，建立了稳定性与分辨率之间的基本权衡关系，为算法设计和性能评估提供了理论基础。
...
TL;DR: 提出了一个无需参数微调的视觉语言模型框架，通过深度感知图像生成将稀疏LiDAR点云转换为2D视觉代理，实现细粒度卡车分类，在少样本场景下达到有竞争力的准确率，并可作为冷启动策略。


<details>
  <summary>Details</summary>
Motivation: 当前基于LiDAR的卡车细粒度分类方法面临可扩展性挑战，需要大量监督学习和人工标注。视觉语言模型具有少样本泛化能力，但存在3D点云与2D图像之间的模态差距问题。

Method: 开发深度感知图像生成流水线，包括噪声去除、时空配准、方向校正、形态学操作和各向异性平滑，将稀疏、遮挡的LiDAR扫描转换为深度编码的2D视觉代理，使用现成VLM无需参数微调。

Result: 在20个车辆类别的真实数据集上，仅需每类16-30个样本即可达到有竞争力的分类准确率。在超少样本场景下文本引导具有正则化效果，但对多样本场景可能因语义不匹配而降低准确率。特定拖车类别达到75%以上的正确分类率。

Conclusion: 该方法为智能交通系统提供了一种可扩展的替代方案，显著减少了初始人工标注的繁重需求，无需昂贵的训练或微调即可实现实用分类性能，特别适合冷启动应用场景。
...
TL;DR: SceneReVis是一个基于视觉的自反思框架，通过迭代的"诊断-行动"循环解决3D场景合成中的空间冲突问题，使用多模态反馈进行空间规划，在生成质量和目标优化方面达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前单次通过的3D场景合成方法缺乏深思熟虑的推理能力，经常出现空间幻觉问题（如碰撞），需要一种能够明确拦截和解决空间冲突的框架。

Method: 提出SceneReVis框架，采用迭代式"诊断-行动"循环，利用多模态反馈解决空间冲突；构建SceneChain-12k大规模数据集；采用两阶段训练方法（监督微调+智能体强化学习），将模型发展为主动空间规划器。

Result: 实验表明SceneReVis在高保真生成和目标导向优化方面达到最先进性能，并在长尾领域展现出强大的泛化能力。

Conclusion: SceneReVis通过自反思框架和多模态反馈有效解决了3D场景合成中的空间冲突问题，为空间规划提供了新的解决方案，在多个方面都表现出优越性能。
...
TL;DR: Fine-T2I是一个大规模、高质量、完全开放的文本到图像微调数据集，包含600万对文本-图像对，覆盖10个任务组合、32个提示类别、11种视觉风格和5种提示模板，通过严格过滤确保质量，显著提升生成质量和指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 当前公开的文本到图像微调数据集存在分辨率低、文本-图像对齐差、多样性有限等问题，导致开源研究模型与企业级模型之间存在明显性能差距，需要高质量开放数据集来缩小这一差距。

Method: 结合现代强模型生成的合成图像和专业摄影师精心策划的真实图像，对所有样本进行文本-图像对齐、视觉保真度和提示质量的严格过滤，淘汰了超过95%的初始候选样本。

Result: 最终数据集包含超过600万对文本-图像对，磁盘占用约2TB，在多种预训练扩散和自回归模型上微调后，通过人工评估、视觉比较和自动指标验证，一致提高了生成质量和指令遵循能力。

Conclusion: Fine-T2I作为大规模高质量开放数据集，有助于缩小文本到图像微调领域的数据差距，推动开源社区发展，数据集已以开放许可发布。
...
TL;DR: 本综述系统回顾了基于深度学习的城市视觉污染检测研究，发现当前研究主要使用YOLO、Faster R-CNN和EfficientDet等架构，但存在数据集局限、缺乏标准分类和地理分布不均等问题，提出了集成视觉污染指数的综合管理框架。


<details>
  <summary>Details</summary>
Motivation: 城市视觉污染已成为重要问题，但自动检测和应用研究仍处于碎片化状态，需要系统梳理现有深度学习方法和构建综合应用框架。

Method: 遵循PRISMA-ScR指南，系统检索了7个学术数据库（Scopus、Web of Science等），筛选出26篇文献进行范围综述分析。

Result: 发现大多数研究专注于特定污染物类别，使用有限的深度学习架构，数据集局限于特定区域且缺乏标准化分类，实时应用系统研究较少且地理分布不均。

Conclusion: 需要建立统一的UVP管理系统，包括污染物分类标准、跨城市基准数据集、通用深度学习模型和评估指标，以支持可持续城市美学和提升居民福祉。
...
TL;DR: 提出两种无需训练的在潜在轨迹调整方法（Look-Ahead和Look-Back），通过在潜在空间直接优化生成路径来减少误差累积，显著提升图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于流匹配的扩散模型通过调整速度场来改进生成过程，但这种方法会导致误差在生成路径中传播。相比之下，在潜在轨迹上的调整能够被预训练的速度网络自然校正，从而减少误差累积。

Method: 提出两种互补的无需训练方法：1) Look-Ahead：使用曲率门控权重平均当前和下一步潜在表示；2) Look-Back：使用指数移动平均衰减平滑潜在表示。两种方法都基于未来和过去的速度和潜在轨迹信息。

Result: 在COCO17、CUB-200和Flickr30K等多个数据集上的广泛实验表明，所提出的轨迹平滑方法在各种评估指标上显著优于最先进模型。

Conclusion: 通过直接在潜在空间调整轨迹而非速度场，能够有效减少误差传播，提供了一种高效且无需重新训练的图像生成质量提升方案。
...
TL;DR: ArtifactLens是一个基于预训练视觉语言模型的合成图像伪影检测系统，仅需每类别数百个标注样本即可达到最先进性能，大幅减少数据需求。


<details>
  <summary>Details</summary>
Motivation: 当前检测器需要数万标注图像进行微调，成本高昂且难以适应生成器演进和新伪影类型。研究发现预训练VLM已具备检测伪影所需知识，只需适当架构即可解锁。

Method: 采用多组件架构，结合上下文学习和文本指令优化，包含新颖改进。系统可泛化到多种伪影类型（物体形态、动物解剖、实体交互）和AIGC检测任务。

Result: 在五个人工伪影基准测试中达到state-of-the-art性能，是首个跨多数据集的评估，同时所需标注数据量减少数个数量级。

Conclusion: 预训练VLM蕴含丰富的伪影检测知识，通过精心设计的架构和少量标注即可有效解锁，为生成器基准测试和改进提供了高效解决方案。
...
TL;DR: FD-DB是一个频率解耦的双分支模型，通过分离低频可解释编辑和高频残差补偿来解决合成到真实图像转换中真实感与结构稳定性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 合成数据为几何敏感视觉任务提供低成本标注样本，但合成域与真实域之间的外观和成像差异导致严重的域偏移，降低下游性能。现有的无配对合成到真实转换方法在真实感和结构稳定性之间存在权衡。

Method: 提出频率解耦的双分支模型：可解释分支预测物理意义的编辑参数（白平衡、曝光、对比度、饱和度、模糊和颗粒）构建稳定的低频外观基础；自由分支通过残差生成补充细节；门控融合机制在明确频率约束下结合两个分支。采用两阶段训练策略。

Result: 在YCB-V数据集上的实验表明，FD-DB提高了真实域外观一致性，显著提升下游语义分割性能，同时保持几何和语义结构。

Conclusion: FD-DB通过频率解耦和双分支设计有效解决了合成到真实转换中的域偏移问题，在保持结构稳定性的同时实现更好的真实感，提升了下游任务的性能。
...
TL;DR: 提出一种名为WeakSupCon的弱监督对比学习框架，用于数字病理学图像分析，通过利用袋级标签信息来改进特征表示学习，从而提升多示例学习性能。


<details>
  <summary>Details</summary>
Motivation: 数字病理学全切片图像分析面临标注数据有限的问题，现有MIL方法主要关注特征聚合而忽略了特征表示学习，特别是缺乏对袋级标签信息的有效利用。

Method: 提出WeakSupCon框架，在不依赖实例级伪标注的情况下，通过弱监督对比学习将不同标签的斑块在特征空间中有效分离，改进图像编码器的预训练过程。

Result: 在三个数据集上的实验结果表明，相比自监督对比学习方法，WeakSupCon生成的特征能够显著提升下游MIL任务的性能表现。

Conclusion: WeakSupCon方法有效解决了数字病理学图像分析中特征表示学习的挑战，为弱监督学习提供了一种新的解决方案，代码已开源。
...
TL;DR: Align-TI是一个新颖的知识蒸馏框架，通过关注token交互来压缩多模态大语言模型，在保持高性能的同时大幅减少模型参数量


<details>
  <summary>Details</summary>
Motivation: 现有的知识蒸馏方法主要依赖静态的下一个token对齐，忽略了动态token交互中嵌入的多模态理解和生成能力，导致蒸馏效果受限

Method: 提出两个核心组件：IVA（指令-视觉对齐）让学生模型模仿教师模型提取相关视觉信息的能力；TPA（token概率对齐）捕获教师模型的动态生成逻辑

Result: 在实验中显著优于传统蒸馏方法（相对提升2.6%），蒸馏后的2B参数模型甚至超越了7B参数的LLaVA-1.5模型（提升7.0%）

Conclusion: Align-TI通过关注token交互实现了高效的多模态模型压缩，为训练参数高效的多模态大语言模型建立了新的最先进蒸馏框架
...
TL;DR: 提出OSI方法，通过将水印提取重新定义为可学习的符号分类问题，实现一步快速提取高斯着色水印，比传统多步扩散反演方法快20倍且精度更高


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯着色的训练免费水印方法需要计算昂贵且耗时的多步扩散反演来获取精确初始噪声，限制了实际应用

Method: 从扩散主干网络初始化OSI模型，在合成的噪声-图像对上使用符号分类目标进行微调，将水印提取转化为符号分类问题

Result: OSI方法比多步扩散反演快20倍，提取准确率更高，水印有效载荷容量翻倍，在不同调度器、扩散主干和密码方案上均表现一致改进

Conclusion: OSI框架提供了一种高效准确的水印提取解决方案，具有广泛的通用性和实用性，解决了传统方法计算成本高的问题
...
TL;DR: ECL是一种针对不平衡数据集的监督对比学习框架，通过促进表示几何平衡和分类器-类中心几何平衡，解决了现有方法中类均值/原型与分类器不对齐以及原型贡献不平衡的问题，在多个长尾和医学数据集上表现优于现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有监督对比学习方法存在两个主要限制：1) 未考虑类均值/原型与分类器的对齐，导致泛化性能差；2) 原型仅作为每个类的一个额外样本，其贡献依赖于批次中类实例数量，造成跨类贡献不平衡。

Method: ECL框架包含两个核心组件：1) 促进表示几何平衡（类样本坍缩和类均值均匀分布的正则单纯形几何），平衡类平均特征和类原型的贡献；2) 建立分类器-类中心几何平衡，对齐分类器权重和类原型。

Result: 在CIFAR-10/100-LT、ImageNet-LT以及ISIC 2019和LCCT医学数据集上的实验表明，ECL优于现有的SOTA监督对比学习方法。

Conclusion: ECL通过实现几何平衡，有效解决了不平衡数据集上的监督对比学习问题，特别是在类均值/原型与分类器对齐以及原型贡献平衡方面取得了显著改进。
...
TL;DR: AdaDS是一个用于深度超分辨率的通用框架，通过扩散模型的逆向过程自适应处理任意退化的低分辨率输入，实现鲁棒的高分辨率深度图恢复。


<details>
  <summary>Details</summary>
Motivation: 传统深度超分辨率方法直接回归深度值，在严重或未知退化条件下容易产生伪影。AdaDS利用高斯平滑的收缩特性，通过扩散过程减少退化输入与高质量目标之间的分布差异。

Method: 基于估计的细化不确定性自适应选择逆向扩散轨迹的起始时间步，注入定制噪声将中间样本定位到目标后验分布的高概率区域，利用预训练扩散模型的生成先验主导恢复过程。

Result: 在真实世界和合成基准测试中，AdaDS展现出卓越的零样本泛化能力和对各种退化模式的鲁棒性，优于现有最先进方法。

Conclusion: AdaDS框架通过扩散模型的逆向过程实现了对任意退化深度输入的鲁棒超分辨率恢复，为解决深度图像处理中的退化问题提供了有效解决方案。
...
TL;DR: 基于帧差法的AI分类器在IoT快速目标检测中的应用，相比端到端方法显著提升精度、能效并降低延迟


<details>
  <summary>Details</summary>
Motivation: IoT系统需要高效节能的快速目标检测应用，而端到端方法在快速目标检测中存在性能不足的问题

Method: 采用帧差法结合AI分类器，在AMD Alveo U50、Jetson Orin Nano和Hailo-8T三种边缘设备上测试了包括MobileNet和YOLOX在内的四种神经网络模型

Result: MobileNet模型表现最佳：高精度、低延迟、高能效；相比端到端方法平均精度提升28.314%，能效提升3.6倍，延迟降低39.305%；快速移动物体（火车、飞机）检测精度相对较低

Conclusion: 提出的轻量级检测算法特别适合需要快速移动目标检测和高精度的IoT应用，帧差法是IoT系统中最高效的快速目标检测方法
...
TL;DR: 构建大规模通用动作空间(UAS)用于分析和分类哺乳动物及黑猩猩行为数据集，基于ImageNet启发的深度学习范式


<details>
  <summary>Details</summary>
Motivation: 传统动物和人类行为分析方法依赖手工特征和稀疏轨迹建模，存在鲁棒性和泛化性限制，需要利用深度学习实现从低级处理到高级表示学习的转变

Method: 利用现有标注的人类动作数据集构建大规模通用动作空间(UAS)，并将其作为基础来分析哺乳动物和黑猩猩行为数据集

Result: 成功构建了通用动作空间，可用于跨物种行为分析，源代码已在GitHub开源

Conclusion: 基于深度学习的大规模动作表示学习为跨物种行为分析提供了有效解决方案，延续了ImageNet在视觉识别领域的成功范式
...
TL;DR: 提出一种无需训练的关注干预算法，通过基于视觉-文本相似度重新分配注意力权重，减少大视觉语言模型的幻觉问题，同时保持生成内容的准确性和连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型存在视觉注意力不足导致的幻觉问题，现有方法增强所有视觉token的注意力会不可避免地增加对任务无关token的关注。

Method: 基于任务相关token通常具有高视觉-文本相似度的假设，提取视觉-文本交叉注意力子矩阵构建重加权矩阵来重新分配注意力；在波束搜索解码中注入视觉注意力值以增强视觉token的贡献。

Result: 大量实验表明该方法显著减少了主流LVLM的幻觉现象

Conclusion: 该方法有效解决了LVLM的幻觉问题，在保持内容准确性和连贯性的同时，通过选择性增强任务相关token的注意力来优化模型性能。
...
TL;DR: Singpath-VL是一个专为宫颈细胞学设计的视觉-语言大模型，通过三阶段合成百万级图像-描述数据集并微调Qwen3-VL-4B模型，在细胞形态感知和诊断分类方面表现优异


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在计算病理学领域取得进展，但在宫颈细胞病理学中应用不足，主要由于缺乏大规模高质量标注数据集

Method: 开发三阶段流水线合成百万级图像-描述数据集：利用通用MLLM作为弱标注器，通过共识融合和专家知识注入精炼输出；采用多阶段策略微调Qwen3-VL-4B模型

Result: Singpath-VL在细粒度形态感知和细胞级诊断分类方面表现出卓越性能

Conclusion: 该模型填补了宫颈细胞学AI助手空白，将开源部分合成数据集和基准测试以推动领域发展
...
TL;DR: HLGFA是一种无监督工业异常检测框架，通过建模高分辨率与低分辨率表示之间的跨分辨率特征一致性来学习正常性，而非依赖像素级重建，在MVTec AD数据集上达到97.9%像素级AUROC和97.5%图像级AUROC的优异性能。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测中缺陷样本稀缺，需要可靠的无监督检测方法。传统方法依赖像素级重建存在局限性，需要更有效的特征对齐方法来识别异常。

Method: 提出高-低分辨率引导特征对齐框架：使用共享冻结骨干网络处理双分辨率输入提取多级特征；将高分辨率表示分解为结构和细节先验，通过条件调制和门控残差校正指导低分辨率特征细化；引入噪声感知数据增强策略抑制工业环境中的干扰响应。

Result: 在标准基准测试中表现优异：MVTec AD数据集上达到97.9%像素级AUROC和97.5%图像级AUROC，优于代表性的基于重建和基于特征的方法。

Conclusion: HLGFA通过跨分辨率特征一致性建模有效解决了工业异常检测问题，无需依赖像素级重建，对工业环境中的噪声干扰具有鲁棒性，为无监督异常检测提供了新的有效解决方案。
...
TL;DR: SchröMind是一个通过求解薛定谔桥问题来减少多模态大语言模型幻觉的新框架，在POPE和MME基准测试中实现了最先进性能，且计算开销极小。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医疗等高风险领域应用受限，主要因为存在幻觉问题，生成的文本会与视觉输入矛盾或忽略视觉信息。模型能理解图像但难以产生准确的token序列，微小的扰动就会导致注意力从真实状态转向虚假状态。

Method: 提出SchröMind框架，通过求解薛定谔桥问题建立幻觉激活和真实激活之间的token级映射，以最小传输成本实现轻量级训练，同时保持模型原有能力。

Result: 在POPE和MME基准测试上进行了广泛实验，证明了SchröMind的优越性，实现了最先进的性能表现。

Conclusion: SchröMind通过薛定谔桥方法有效解决了多模态大语言模型的幻觉问题，在保持模型能力的同时显著提升了生成文本的准确性，为高风险领域的应用提供了可行解决方案。
...
TL;DR: SCA-Net是一个基于Change-Agent框架的增强型建筑和道路变化检测架构，通过多尺度差异金字塔、自适应多尺度处理和注意力机制等创新，在LEVIR数据集上实现了显著的性能提升和小目标检测改进，同时大幅减少训练时间。


<details>
  <summary>Details</summary>
Motivation: 遥感影像中的自动化变化检测对于城市管理、环境监测和灾害评估至关重要。尽管深度学习模型推动了该领域发展，但仍面临小目标敏感性低和计算成本高等挑战。

Method: 提出SCA-Net架构，包含：差异金字塔块进行多尺度变化分析；自适应多尺度处理模块结合形状感知和高分辨率增强块；多级注意力机制（PPM和CSAGate）进行联合上下文和细节处理；动态复合损失函数和四阶段训练策略。

Result: 在LEVIR-CD和LEVIR-MCI数据集上，SCA-Net相比Change-Agent和其他SOTA方法表现更优：LEVIR-MCI的mIoU提升2.64%，小建筑IoU显著提高57.9%，训练时间减少61%。

Conclusion: 该工作为实际变化检测应用提供了一个高效、准确且鲁棒的解决方案，在性能指标和计算效率方面均有显著改进。
...
TL;DR: DR.Experts是一个基于失真先验的盲图像质量评估框架，通过退化感知视觉语言模型获取失真特定先验，利用失真显著性差异模块增强表示，并通过动态失真加权模块融合特征，显著提升了与人类感知的对齐性能。


<details>
  <summary>Details</summary>
Motivation: 现有盲图像质量评估模型难以有效捕捉细微失真线索，导致与人类主观判断不一致。根本原因在于缺乏可靠的失真先验，模型仅学习图像特征与质量分数的浅层关系，对失真不敏感。

Method: 1. 使用退化感知视觉语言模型获取失真特定先验
2. 提出失真显著性差异模块，区分语义注意力以增强失真表示
3. 设计动态失真加权模块，按感知重要性加权各失真特征
4. 融合语义和桥接表示进行最终质量预测

Result: 在五个具有挑战性的BIQA基准测试上进行广泛实验，证明DR.Experts优于当前方法，在泛化能力和数据效率方面表现出色。

Conclusion: 通过显式引入失真先验并有效融合多源特征，DR.Experts框架成功解决了传统BIQA模型对失真不敏感的问题，实现了与人类感知更好的对齐，为盲图像质量评估提供了新的有效解决方案。
...
TL;DR: RAD是一个检索增强的单目深度估计框架，通过检索相似场景的RGB-D样本来提升对罕见类别的深度估计精度，在多个基准测试中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计在处理复杂场景中罕见类别时存在精度不足的问题，需要一种能够利用外部几何信息的方法来提升性能。

Method: 采用不确定性感知检索机制识别输入图像中的低置信度区域，检索语义相似的RGB-D样本作为几何代理，通过双流网络和匹配交叉注意力模块进行信息融合。

Result: 在NYU Depth v2上相对绝对误差降低29.2%，KITTI上降低13.3%，Cityscapes上降低7.2%，在罕见类别上表现显著提升。

Conclusion: RAD框架通过检索增强策略有效解决了单目深度估计中罕见类别的精度问题，证明了利用外部几何信息的重要性。
...
TL;DR: AUHead是一种两阶段方法，通过动作单元(AU)实现细粒度情感控制的表情头部视频生成。第一阶段使用音频语言模型从语音中解耦AU，第二阶段通过AU驱动的扩散模型合成高质量视频。


<details>
  <summary>Details</summary>
Motivation: 当前方法缺乏细粒度情感控制能力，难以生成具有微妙情感表达的真实说话头部视频。

Method: 1. 第一阶段：空间-时间AU标记化和"情感-然后-AU"思维链机制，从原始语音中解耦AU
2. 第二阶段：AU驱动的可控扩散模型，将AU序列映射为结构化2D面部表示，通过交叉注意力模块建模AU-视觉交互，推理时使用AU解耦引导策略

Result: 在基准数据集上取得竞争性表现，在情感真实性、准确唇形同步和视觉连贯性方面显著超越现有技术。

Conclusion: 该方法成功实现了细粒度情感控制，为虚拟化身、电影制作和交互系统提供了更真实的说话头部视频生成解决方案。
...
TL;DR: Scalpel是一种无需额外计算、在推理过程中动态调整注意力激活分布以减少大视觉语言模型幻觉的方法，通过高斯混合模型和熵最优传输技术实现，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型由于大语言模型的强先验和跨模态注意力错配，经常产生与视觉内容不一致的幻觉输出，需要有效解决方案。

Method: 提出Scalpel方法：1）预测Transformer层中每个注意力头的可信注意力方向；2）使用高斯混合模型捕捉信任和幻觉流形的多峰分布；3）通过熵最优传输（等价于薛定谔桥问题）精确映射高斯分量；4）基于分量隶属度和映射关系动态调整干预强度和方向。

Result: 在多个数据集和基准测试中，Scalpel有效减轻了幻觉现象，性能优于先前方法，达到state-of-the-art水平。

Conclusion: Scalpel是一种模型无关、数据无关的幻觉缓解方法，仅需单次解码步骤，无需额外计算开销，为大视觉语言模型的幻觉问题提供了有效解决方案。
...
TL;DR: 提出基于神经正切核的多模态谱聚类方法，利用预训练视觉语言模型的跨模态对齐能力，通过语义正名词增强图像间亲和力计算，并引入正则化亲和力扩散机制整合多提示词生成的亲和矩阵。


<details>
  <summary>Details</summary>
Motivation: 传统谱聚类方法主要基于单模态信息，未能充分利用多模态表示的丰富信息。受视觉语言预训练模型成功的启发，希望将谱聚类从单模态扩展到多模态领域。

Method: 提出神经正切核谱聚类方法，使用语义上接近目标图像的正名词作为锚点，将图像间亲和力计算结合视觉邻近性和语义重叠性。引入正则化亲和力扩散机制自适应地整合不同提示词生成的亲和矩阵。

Result: 在16个基准数据集（包括经典、大规模、细粒度和域偏移数据集）上的广泛实验表明，该方法大幅优于现有最先进方法。

Conclusion: 该方法成功将谱聚类扩展到多模态领域，通过跨模态对齐和正则化扩散机制显著提升了聚类性能，验证了多模态信息在谱聚类中的重要性。
...
TL;DR: MieDB-100k是一个大规模、高质量、多样化的医学图像编辑数据集，通过专业数据构建流程解决现有医学图像编辑数据稀缺、多样性不足和质量可扩展性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像编辑数据集存在多样性有限、忽视医学图像理解能力以及无法平衡质量与可扩展性的问题，这成为多模态生成模型在医学图像编辑应用中的主要瓶颈。

Method: 通过数据策展流程构建数据集，结合模态特异性专家模型和基于规则的数据合成方法，并进行严格的人工检查以确保临床保真度。将编辑任务分为感知、修改和转换三个维度，兼顾理解和生成能力。

Result: 实验表明，使用MieDB-100k训练的模型在性能上持续优于开源和专有模型，并展现出强大的泛化能力。

Conclusion: MieDB-100k数据集有望成为专业医学图像编辑领域未来发展的基石，为解决医学图像编辑数据稀缺问题提供了有效解决方案。
...
TL;DR: Hand2World是一个统一的自动回归框架，用于从单张场景图像和自由空间手势生成以自我为中心的交互视频，通过遮挡不变的手部条件化、显式相机几何注入和双向扩散模型蒸馏来解决分布偏移、运动模糊和长时程生成挑战。


<details>
  <summary>Details</summary>
Motivation: 自我中心交互世界模型对于增强现实和具身AI至关重要，需要低延迟、几何一致性和长期稳定性的视觉生成。现有方法面临自由空间手势与接触式训练数据之间的分布偏移、单目视图中手部运动与相机运动的模糊性，以及任意长度视频生成的挑战。

Method: 1. 基于投影3D手部网格的遮挡不变手部条件化，从场景上下文推断可见性和遮挡；2. 通过每像素Plücker射线嵌入注入显式相机几何，解耦相机运动与手部运动；3. 开发全自动单目标注流程；4. 将双向扩散模型蒸馏为因果生成器，支持任意长度合成。

Result: 在三个自我中心交互基准测试中，感知质量和3D一致性显著提升，同时支持相机控制和长时程交互生成。

Conclusion: Hand2World通过创新的遮挡处理、相机运动解耦和模型蒸馏技术，有效解决了自我中心交互生成的关键挑战，为增强现实和具身AI应用提供了高质量的视觉生成解决方案。
...
TL;DR: Tele-Omni是一个统一的多模态视频生成与编辑框架，能够处理文本、图像和参考视频等多种输入，通过解耦指令解析与视频合成实现灵活的跨任务控制。


<details>
  <summary>Details</summary>
Motivation: 现有扩散视频生成方法多为任务特定且主要依赖文本指令，无法统一处理多模态输入和上下文参考，且视频编辑方法依赖针对特定操作的工程化流程，限制了可扩展性和组合性。

Method: 利用预训练多模态大语言模型解析异构指令并推断结构化生成/编辑意图，基于扩散的生成器根据结构化信号进行高质量视频合成，引入任务感知数据处理管道统一多模态输入格式。

Result: 实验结果表明Tele-Omni在多个任务上达到竞争性性能，支持文本到视频生成、图像到视频生成、首尾帧视频生成、上下文视频生成和编辑等多种视频中心任务。

Conclusion: 通过解耦指令解析与视频合成并结合任务感知数据设计，Tele-Omni实现了灵活的多模态控制，同时保持了强时间连贯性和视觉一致性，为统一视频生成与编辑提供了有效解决方案。
...
TL;DR: AGMark提出了一种注意力引导的动态水印框架，通过动态识别视觉相关性的语义关键证据和上下文一致性线索，结合不确定性感知和证据校准，在保持视觉保真度的同时实现高效水印检测。


<details>
  <summary>Details</summary>
Motivation: 现有视觉无关水印会引入视觉无关标记破坏视觉基础，而视觉特定水印采用静态权重估计，忽略权重分布密度和生成过程中视觉依赖的动态变化，导致长尾低质量标记。

Method: 在每个解码步骤中：1）基于注意力权重动态识别视觉相关性的语义关键证据；2）结合上下文一致性线索获得自适应校准的证据权重分布；3）联合考虑不确定性感知（标记熵）和证据校准（权重密度）确定语义关键标记比例；4）实现自适应词汇分区避免无关标记。

Result: AGMark在生成质量上显著优于传统方法，特别是在生成后期阶段视觉语义保真度提升明显。保持高竞争性检测准确率（至少99.36% AUC）和强健攻击抗性（至少88.61% AUC），且不牺牲推理效率。

Conclusion: AGMark有效解决了现有水印方法的局限性，通过动态注意力引导机制实现了可靠保护多模态水印的新标准，在保持视觉保真度的同时确保了水印的可检测性和抗攻击能力。
...
TL;DR: LELA是首个无需训练的基于大语言模型的仇恨视频定位框架，通过多模态分解和多阶段提示机制实现细粒度的仇恨内容检测和时序定位，在多个基准测试中显著优于现有无训练基线方法。
...
Result: 在HateMM和MultiHateClip两个挑战性基准测试中，LELA大幅优于所有现有的无训练基线方法，并通过大量消融实验和定性可视化验证了其有效性。

Conclusion: LELA为可扩展和可解释的仇恨视频定位奠定了坚实基础，提供了一种无需训练的高效解决方案。
...
TL;DR: VIDA数据集和VideoAfford模型：通过视频交互数据增强3D可操作性理解，统一多模态大语言模型与细粒度分割能力，显著提升开放世界泛化性能


<details>
  <summary>Details</summary>
Motivation: 现有3D可操作性研究主要依赖静态语言和图像线索，缺乏动态交互上下文中的时序和因果信息，限制了机器人对物体可操作区域的准确理解

Method: 构建VIDA数据集（38K人-物交互视频，16种可操作性类型，38个物体类别，22K点云）；开发VideoAfford模型，集成多模态大语言模型与潜在动作编码器，引入空间感知损失函数提取动态交互先验

Result: 模型在实验评估中显著超越现有方法，展现出强大的开放世界泛化能力和可操作性推理能力

Conclusion: 视频交互数据能有效提供动态上下文信息，统一框架结合世界知识推理与细粒度分割可实现更准确的3D可操作性理解，为机器人操作提供重要技术基础
...
TL;DR: Time2General是一个领域泛化视频语义分割框架，通过时空记忆解码器和掩码时序一致性损失来解决域偏移和时序采样偏移问题，显著提升跨域精度和时序稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的视频语义分割方法在跨域部署时面临域偏移和时序采样偏移的双重挑战，导致基于对应关系传播和固定步长时序聚合的方法失效，产生严重的帧间闪烁问题。

Method: 提出基于稳定性查询的Time2General框架，包含：1）时空记忆解码器，聚合多帧上下文到片段级时空记忆并解码时序一致的每帧掩码；2）掩码时序一致性损失，正则化不同步长的时序预测差异；3）随机化训练步长以暴露模型于多样化时序间隔。

Result: 在多个驾驶基准测试中，Time2General相比现有DGSS和VSS基线方法，在跨域精度和时序稳定性方面取得显著提升，同时达到最高18 FPS的运行速度。

Conclusion: 该方法有效解决了领域泛化视频语义分割中的时序一致性问题，无需目标域标签和测试时适应，为实际部署提供了高效稳定的解决方案。
...
TL;DR: TreeCUA：基于树结构可验证演化的GUI自动化扩展方法，通过多智能体协作框架和树形拓扑结构高效生成高质量GUI轨迹，并提出了TreeCUA-DPO方法提升GUI规划能力


<details>
  <summary>Details</summary>
Motivation: 现有GUI自动化研究主要关注GUI定位而非更关键的GUI规划，且GUI探索过程天然呈现树状结构，需要更高效的数据收集和扩展方法

Method: 提出多智能体协作框架进行环境探索、动作验证、轨迹总结和质量评估；设计树形拓扑存储重复探索节点；开发自适应探索算法平衡深度与广度；采用世界知识引导和全局记忆回溯避免低质量生成；基于树节点信息扩展出TreeCUA-DPO方法

Result: 实验结果显示TreeCUA和TreeCUA-DPO方法显著提升性能，跨域研究证明了强泛化能力

Conclusion: TreeCUA通过树结构组织和演化GUI轨迹，有效解决了GUI规划的数据扩展问题，为计算机使用智能体的GUI自动化提供了高效可扩展的解决方案
...
TL;DR: 基于CARE Liver 2025 Track 4挑战，本研究提出了一个多任务深度学习框架，用于多参数MRI的肝脏分割(LiSeg)和肝纤维化分期(LiFS)，通过半监督学习和图像配准技术解决标注数据有限和多模态数据复杂性问题。


<details>
  <summary>Details</summary>
Motivation: 肝纤维化在临床实践中是一个重大挑战，需要精确的肝脏分割和准确的疾病分期，而多参数MRI数据的标注有限和领域偏移问题亟待解决。

Method: 采用半监督学习模型整合图像分割和配准技术处理LiSeg任务；使用基于patch的方法进行LiFS任务，实现肝纤维化分期的可视化。模型处理三通道(T1,T2,DWI)和七通道(T1,T2,DWI,GED1-GED4)MRI数据。

Result: 方法在独立测试集上进行了验证，包括分布内(ID)和分布外(OOD)病例，有效处理了多模态成像数据、有限标签和领域偏移问题。

Conclusion: 提出的多任务框架成功解决了肝脏分割和肝纤维化分期的关键挑战，代码已开源，为临床实践提供了有效的深度学习解决方案。
...
TL;DR: GenSeg-R1是一个解耦的'先推理后分割'框架，使用视觉语言模型进行场景推理生成结构化空间提示，然后由冻结的SAM 2分割器生成高质量掩码，在多个基准测试中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决细粒度参考图像分割问题，通过解耦的推理-分割流程避免需要监督推理链标注，同时提升分割质量和负样本检测能力。

Method: 使用Qwen3-VL模型(4B和8B参数)进行视觉语言推理，生成边界框和内部关键点作为空间提示，通过Group Relative Policy Optimization (GRPO)进行微调，无需监督推理链标注。还开发了GenSeg-R1-G变体，使用SAM 2在线奖励直接优化掩码质量。

Result: 在RefCOCOg验证集上，GenSeg-R1-8B达到0.7127 cIoU和0.7382 mIoU，显著超越基准方法；在GRefCOCO验证集上，GenSeg-R1-G达到76.69%目标mIoU和82.40%负样本检测准确率；在ReasonSeg测试集上，GenSeg-R1-4B达到68.40% mIoU。

Conclusion: 提出的解耦推理-分割框架在多个基准测试中表现出色，特别是在负样本检测和分割质量方面显著优于现有方法，证明了无需监督推理链标注的有效性。
...
TL;DR: Stroke3D是一个从用户输入的2D笔划和文本提示直接生成可动画绑定3D网格的新框架，采用两阶段流程：可控骨架生成和增强网格合成


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法难以生成可动画的几何体，而绑定技术缺乏对骨架创建的细粒度结构控制，需要更直观的工作流程来创建即用型3D内容

Method: 两阶段流程：1) 使用Skeletal Graph VAE和DiT进行可控骨架生成，基于文本语义和2D笔划进行条件生成；2) 通过TextuRig数据集增强和SKA-DPO偏好优化策略进行网格合成

Result: 框架能够生成合理的骨架和高质量网格，实验证明Stroke3D在生成可动画3D内容方面表现优异

Conclusion: Stroke3D首次实现了基于用户绘制2D笔划的条件化绑定3D网格生成，为创建即用型动画内容提供了更直观的工作流程
...
TL;DR: 本研究首次系统性地对轻量级SNN进行了基准测试，通过将紧凑型CNN架构转换为脉冲神经网络，在保持竞争性精度的同时实现了最高15.7倍的能效提升，并通过结构化剪枝进一步优化模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注大规模SNN模型，轻量级CNN到SNN转换流程的设计和评估尚未得到充分探索，而边缘智能应用迫切需要高能效的轻量级神经网络解决方案。

Method: 采用泄漏积分发放(LIF)神经元建模激活，使用替代梯度下降法在统一设置下训练，构建了ShuffleNet、SqueezeNet、MnasNet和MixNet的脉冲变体，并在CIFAR-10、CIFAR-100和TinyImageNet数据集上评估精度、F1分数、参数量、计算复杂度和能耗。

Result: SNN相比CNN实现了最高15.7倍的能效提升，其中SNN-SqueezeNet表现最佳。通过结构化剪枝得到的SNN-SqueezeNet-P模型在CIFAR-10上精度提升6%，参数量减少19%，与CNN-SqueezeNet精度差距缩小至仅1%，同时能耗降低88.1%。

Conclusion: 轻量级SNN是边缘部署的实用低功耗替代方案，为在边缘设备上部署高性能、低功耗智能系统提供了可行路径。
...
TL;DR: 提出了一种混合方法，将裂纹检测建模为逆问题，通过深度生成模型和变分函数联合优化，实现绘画中裂纹的像素级定位。


<details>
  <summary>Details</summary>
Motivation: 自动化检测数字化绘画中的裂纹对于评估退化和指导修复至关重要，但由于复杂场景以及裂纹与类似艺术特征（如笔触或发丝）的视觉相似性，该任务仍然具有挑战性。

Method: 将裂纹检测建模为逆问题，将观察图像分解为无裂纹绘画和裂纹成分。使用深度生成模型作为底层艺术品的强大先验，同时使用Mumford-Shah型变分函数和裂纹先验捕获裂纹结构。通过联合优化获得裂纹的像素级定位图。

Result: 该方法能够有效分离裂纹与类似艺术特征，实现精确的裂纹检测。

Conclusion: 该混合方法为艺术品文档化和保护提供了有效的非侵入性分析工具，特别是在处理复杂艺术场景中的裂纹检测方面表现出色。
...
TL;DR: FG-3DGS是一种新颖的音频驱动说话人生成框架，通过频率感知解耦策略分别处理低频和高频面部区域，结合后渲染对齐机制，实现了高保真、唇同步精确的3D高斯溅射说话人生成。


<details>
  <summary>Details</summary>
Motivation: 解决现有3D高斯溅射方法在说话人生成中存在的唇同步不准确和面部抖动问题，这些缺陷会导致恐怖谷效应，需要实现更精细的面部运动控制。

Method: 采用频率感知解耦策略：低频区域（脸颊、鼻子、额头）使用标准MLP建模，高频区域（眼睛、嘴巴）使用专用网络配合面部区域掩码。通过高斯增量表示运动动态，结合后渲染对齐机制提升唇同步精度。

Result: 在广泛使用的说话人生成数据集上的实验表明，该方法在生成高保真、唇同步的说话人视频方面优于现有最先进方法。

Conclusion: FG-3DGS框架通过频率感知的区域解耦和后渲染对齐机制，成功解决了面部精细运动控制的挑战，实现了时间一致的高质量说话人生成，为数字化身技术提供了有效解决方案。
...
TL;DR: 论文分析了自动驾驶车辆视觉系统的安全性，提出了CAVVS参考架构来识别攻击面，详细阐述了攻击向量及其对CIA三要素的影响，为制定安全措施提供基础。


<details>
  <summary>Details</summary>
Motivation: 开发L5级自动驾驶需要可靠的视觉系统，但目前缺乏对视觉系统安全性的全面分析，需要识别潜在攻击面并评估安全影响。

Method: 分析关键传感器和视觉组件，构建CAVVS参考架构，识别攻击面，详细分析针对每个攻击面的攻击向量，并评估其对机密性、完整性和可用性的影响。

Result: 建立了完整的CAVVS参考架构，识别了多个攻击面，详细分析了各种攻击向量及其对CIA三要素的具体影响。

Conclusion: 研究提供了对视觉系统攻击向量动态的全面理解，为制定能够维护CIA三原则的强健安全措施奠定了重要基础。
...
TL;DR: 该论文提出了一种离散通信框架的视觉自监督学习方法，通过教师-学生网络和固定容量二进制通道传递语义信息，使用二元交叉熵损失和编码率正则化来学习结构化表示，在多个视觉任务上优于连续对齐方法。


<details>
  <summary>Details</summary>
Motivation: 传统自监督学习方法通过对齐同一输入的不同视图来学习连续视觉表示，但对表示维度中信息的结构化控制有限，需要一种能够学习结构化离散表示的方法。

Method: 将视觉自监督学习构建为教师和学生网络之间的离散通信过程，通过固定容量二进制通道传输语义信息；使用元素级二元交叉熵目标强制离散一致性，编码率正则化项促进受限通道的有效利用；定期重新初始化投影头以增强表示的可预测性。

Result: 在图像分类、检索、密集视觉预测任务以及域偏移下的自监督适应中，相比连续一致性基线方法获得了持续改进；学习到的二进制代码形成了紧凑且信息丰富的离散语言，能够跨类别捕获可重用的语义因子。

Conclusion: 离散通信框架为视觉自监督学习提供了一种有效的方法，能够学习结构化、信息丰富的离散表示，在多个视觉任务上表现出优越性能，并为语义信息的紧凑编码提供了新途径。
...
TL;DR: Code2World是一个基于代码生成的视觉语言编码器，通过生成可渲染的HTML代码来预测GUI界面的下一个视觉状态，解决了现有方法在视觉保真度和结构可控性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于文本和像素的GUI世界模型方法难以同时实现高视觉保真度和细粒度结构可控性，需要一种新的方法来提升自主GUI代理的预测能力。

Method: 1) 构建AndroidCode数据集：将GUI轨迹转换为高质量HTML代码，通过视觉反馈修订机制获得8万+高质量屏幕-动作对；2) 使用监督微调(SFT)进行格式布局跟随；3) 应用渲染感知强化学习，以渲染结果作为奖励信号，确保视觉语义保真度和动作一致性。

Result: Code2World-8B在下一代UI预测方面达到顶级性能，媲美GPT-5和Gemini-3-Pro-Image。在下游导航任务中显著提升成功率，将Gemini-2.5-Flash在AndroidWorld导航上的成功率提升9.5%。

Conclusion: Code2World通过代码生成方法有效解决了GUI世界建模中的视觉保真度和结构可控性问题，为自主GUI代理提供了人类般的预见能力，在多个基准测试中表现出色。
...
TL;DR: 研究通过LLM从图像标题中提取地理位置信息，分析了三个大型多模态数据集的地区代表性，发现美国、英国、加拿大占据48%样本，南美和非洲严重不足，GDP与数据代表性高度相关，且高代表性不等于高多样性，生成模型Stable Diffusion的覆盖范围也有限。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型常无法生成地理代表性图像，引发对训练数据代表性的关注，需要探究这些训练样本来自世界哪些地区。

Method: 使用LLM从图像标题中提取位置信息，将图像-标题对映射到国家，分析三个英文数据集(Re-LAION、DataComp1B、Conceptual Captions)中20个常见实体的地理分布，并研究Re-LAION中4种非英语语言子集，最后分析Stable Diffusion v1.3生成图像的地理覆盖。

Result: 美国、英国、加拿大占48.0%样本，南美和非洲仅占1.8%和3.8%；国家GDP与数据代表性强相关(ρ=0.82)；非英语语言数据偏向主要使用国；高代表性不等于高视觉或语义多样性；Stable Diffusion生成图像虽真实但覆盖范围严重受限。

Conclusion: 大型多模态数据集存在严重的地理偏见，偏向高GDP国家，这种偏见会影响文本到图像模型的生成能力，导致无法充分代表全球多样性，需要更平衡的数据收集策略。
...
TL;DR: SciFlow-Bench是一个针对科学图表生成的结构优先基准测试，通过将生成的图表图像反向解析为结构化图进行对比，强调结构正确性而非仅视觉相似性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型生成的科学图表往往视觉上合理但结构错误，而现有基准测试要么关注图像中心或主观指标，要么评估中间符号表示而非最终渲染图像，导致基于像素的图表生成研究不足。

Method: 从真实科学PDF中构建基准，每个源框架图都配有规范的真实图，通过闭环往返协议将生成的图表图像反向解析为结构化图进行比较，使用分层多智能体系统协调规划、感知和结构推理。

Result: 实验表明保持结构正确性仍然是一个基本挑战，特别是对于具有复杂拓扑结构的图表。

Conclusion: 结构感知评估对于科学图表生成至关重要，SciFlow-Bench提供了一个有效的评估框架来检验模型的结构保持能力。
...
TL;DR: CompSplat：针对真实世界长视频的压缩感知训练框架，通过显式建模帧级压缩特性来解决压缩导致的帧间不一致性和几何误差累积问题，在严重压缩条件下实现最先进的渲染质量和位姿精度


<details>
  <summary>Details</summary>
Motivation: 真实世界视频通常包含长序列、不规则相机轨迹和未知位姿，导致重建过程中的位姿漂移、特征错位和几何失真。有损压缩会放大这些问题，引入不一致性并逐渐降低几何和渲染质量。现有方法对长视频中多样化压缩模式的探索不足

Method: 提出压缩感知训练框架CompSplat，包含压缩感知帧加权策略和自适应剪枝策略，显式建模帧级压缩特性以增强鲁棒性和几何一致性，特别是在重度压缩条件下

Result: 在Tanks and Temples、Free和Hike等挑战性基准测试中，CompSplat实现了最先进的渲染质量和位姿精度，在严重压缩条件下显著超越大多数最新的NVS方法

Conclusion: CompSplat通过压缩感知的方法有效解决了长视频重建中的压缩相关问题，为文化保护、数字孪生和沉浸式媒体等应用提供了高质量的新视角合成解决方案
...
TL;DR: SAKED方法通过分析LVLM内部知识不稳定性来缓解幻觉问题，提出层间知识稳定性评分机制，在无需训练的情况下有效提升模型输出的可靠性。


<details>
  <summary>Details</summary>
Motivation: 受人类在不确定时更容易出错的现象启发，研究大型视觉语言模型内部知识不稳定性如何导致幻觉问题，旨在提高模型的安全性和可靠性。

Method: 从注意力头、模型层和解码token三个角度进行实证分析，识别三种关键幻觉模式，提出基于层间知识稳定性评分(KSS)的稳定性感知知识增强解码方法(SAKED)。

Result: SAKED在不同模型、任务和基准测试中实现了最先进的幻觉缓解性能，且无需训练即可无缝集成到不同架构中。

Conclusion: 通过量化模型内部知识稳定性并动态利用最可靠的知识层，SAKED能有效抑制解码噪声，为忠实token生成提供可靠解决方案，显著降低LVLM的幻觉风险。
...
TL;DR: ARK是一个多模态检索基准，专注于专业知识和复杂推理评估，包含5个知识领域17个子类型和6类推理技能，使用16种视觉数据类型，通过设计困难负样本来避免捷径匹配。评估23个检索模型显示知识密集和推理密集检索存在显著差距，视觉和空间推理是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有多模态检索基准主要关注日常生活图像的语义匹配，缺乏对专业知识和复杂推理的诊断能力。为填补这一空白，需要开发能够分析多模态检索在知识领域和推理技能两个互补维度表现的基准。

Method: 构建ARK基准，包含：1）5个知识领域17个子类型的内容专业知识；2）6类推理技能的多模态证据推断；3）支持单模态和多模态查询与候选；4）16种异构视觉数据类型；5）大多数查询配有需要多步推理的困难负样本以避免捷径匹配。评估23个代表性文本和多模态检索模型。

Result: 评估发现：1）知识密集型检索和推理密集型检索存在显著性能差距；2）细粒度视觉推理和空间推理是持续存在的瓶颈；3）简单的增强方法（如重排序和查询重写）能带来一致改进，但仍有很大提升空间。

Conclusion: ARK基准有效揭示了多模态检索在专业知识和复杂推理方面的挑战，特别是视觉和空间推理能力不足。虽然现有增强方法有一定效果，但该领域仍需进一步研究来克服这些核心瓶颈。
...
TL;DR: Kelix是一种完全离散的自回归统一模型，通过改进的视觉标记化方法解决了离散视觉表示在理解能力上弱于连续特征的问题，实现了多模态理解与生成的统一。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型大多采用混合接口（离散文本标记+连续ViT特征），导致模型偏向于理解任务且无法充分利用非文本数据的大规模自监督学习。虽然近期研究探索了离散视觉标记化，但现有方法因码本容量有限导致信息丢失，理解能力明显弱于连续特征模型。

Method: 提出了Kelix模型，采用完全离散的自回归建模方法，通过改进的视觉标记化技术来提升离散表示的信息保留能力，实现文本和视觉模态的统一离散表示。

Result: Kelix成功缩小了离散视觉表示与连续特征视觉语言模型之间的理解能力差距，实现了统一的多模态理解和生成能力。

Conclusion: 完全离散的自回归建模是多模态AI发展的可行方向，通过改进的离散视觉表示技术可以实现与连续特征相当的理解性能，为统一的多模态模型提供了新的解决方案。
...
TL;DR: Reason-IAD：基于知识引导动态潜在推理的工业异常检测框架，通过检索增强知识模块和熵驱动潜在推理机制，显著提升检测精度和可解释性
...
Method: 包含检索增强知识模块（融入类别特定文本描述）、熵驱动潜在推理机制（在紧凑潜在空间中进行迭代探索）、动态视觉注入策略（选择性整合关键图像块）

Result: 在广泛实验中一致优于最先进方法

Conclusion: 提出的框架有效解决了工业异常检测中的细粒度缺陷模式识别问题，提供了可解释的检测结果
...
TL;DR: Free-GVC是一种无需训练的生成式视频压缩框架，通过潜在轨迹压缩和视频扩散先验，在超低码率下实现优异的感知质量和时间一致性。
...
Method: 采用基于视频扩散先验的潜在轨迹压缩方法，包含自适应质量控制模块动态构建在线率失真代理模型，以及帧间对齐模块通过潜在融合增强时间一致性。

Result: 在DISTS指标上相比最新神经编解码器DCVC-RT平均降低93.29%的码率，用户研究证实其在超低码率下具有优越的感知质量和时间一致性。

Conclusion: Free-GVC框架通过创新的训练自由方法和有效的质量控制机制，为超低码率视频压缩提供了高质量的解决方案，显著提升了生成式视频压缩的性能。
...
TL;DR: BabyMamba-HAR是一个针对资源受限可穿戴设备的人体活动识别框架，包含两种轻量级Mamba架构变体：通道独立版本防止跨通道噪声传播，交叉双向版本实现通道数无关计算复杂度。在8个基准测试中达到86.52%平均F1分数，参数量仅27K，计算量比TinyHAR减少11倍。


<details>
  <summary>Details</summary>
Motivation: 解决可穿戴设备上人体活动识别(HAR)面临的内存和计算资源限制问题，同时需要在异构传感器配置下保持竞争性精度。选择性状态空间模型(SSMs)提供线性时间序列处理能力，但在TinyML领域的部署设计空间尚未充分探索。

Method: 提出两种新颖架构：1) CI-BabyMamba-HAR：使用通道独立主干，通过共享权重但实例独立的变换处理每个传感器通道；2) Crossover-BiDir-BabyMamba-HAR：使用早期融合主干实现通道数无关计算复杂度。两种变体都包含权重绑定的双向扫描和轻量级时序注意力池化。

Result: Crossover-BiDir-BabyMamba-HAR在8个基准测试中达到86.52%平均宏F1分数，仅需约27K参数和2.21M MACs，与TinyHAR性能相当(86.16%)但在高通道数据集上计算量减少11倍。双向扫描贡献最多8.42% F1提升，门控时序注意力比平均池化最多提升8.94% F1分数。

Conclusion: 研究确立了选择性状态空间模型作为高效TinyML骨干网络的实际设计原则，为资源受限环境下的人体活动识别提供了有效的解决方案，证明了SSMs在保持精度的同时显著降低计算复杂度的优势。
...
TL;DR: 提出基于4D世界模型的机器人操作新范式，通过单视角RGBD输入生成多视角几何一致的RGBD序列，并采用测试时动作优化策略实现精确操作控制


<details>
  <summary>Details</summary>
Motivation: 现有方法仅支持纯图像预测或部分3D几何推理，无法预测完整的4D场景动态，限制了机器人操作能力

Method: 设计跨视角和跨模态特征融合机制确保RGB与深度一致性及视角间几何对齐；采用测试时动作优化策略通过生成模型反向传播推断轨迹级潜在变量，配合残差逆动力学模型生成可执行动作

Result: 在三个数据集上验证了4D场景生成和下流操作任务的优异性能，消融实验验证了关键设计选择的有效性

Conclusion: 该方法实现了几何一致的任意视角RGBD生成，显著提升了机器人操作的预测准确性和执行精度，为具身智能提供了有效的4D世界建模解决方案
...
TL;DR: AdaTSQ是一种新颖的DiT后训练量化框架，通过时间敏感度感知的比特宽度分配和Fisher引导的时间校准，显著提升了扩散变换器的量化效率与质量。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiTs)在图像和视频生成中表现出色，但其巨大的计算成本和内存占用阻碍了在边缘设备上的部署。现有的后训练量化方法直接应用于DiTs时效果不佳，因为它们忽略了扩散过程独特的时间动态特性。

Method: 1. 提出Pareto感知的时间步动态比特宽度分配策略：将量化策略搜索建模为约束路径寻找问题，使用端到端重建误差引导的束搜索算法，为不同时间步动态分配层间比特宽度
2. 提出Fisher引导的时间校准机制：利用时间Fisher信息优先选择高敏感时间步的校准数据，与基于Hessian的权重优化无缝集成

Result: 在四个先进DiT模型(Flux-Dev、Flux-Schnell、Z-Image、Wan2.1)上的广泛实验表明，AdaTSQ显著优于SVDQuant和ViDiT-Q等最先进方法

Conclusion: AdaTSQ通过有效利用DiTs的时间动态特性，成功推动了效率与质量的Pareto前沿，为边缘设备部署高保真图像生成模型提供了有效解决方案
...
TL;DR: 提出SARS系统：基于单张图像的形状和外观感知3D人体重建方法，同时考虑全局结构和面部语义特征


<details>
  <summary>Details</summary>
Motivation: 现有3D人体重建方法仅关注全局面部结构或几何形状，忽略了年龄、性别、面部标志点等语义特征，无法适应这些高级面部特征的变化

Method: 开发模块化流水线SARS，从单张图像中提取身体和面部信息，结合身份和表情混合形状与基础面部网格，通过调整形状、纹理、光照和相机参数来控制3D可变形模型的变异性

Result: 成功构建了能够同时重建人体全局结构和面部语义特征的3D模型系统

Conclusion: SARS系统通过整合面部语义特征，实现了更全面和准确的人体3D重建，解决了传统方法忽略高级面部特征的问题
...
TL;DR: LASANA数据集：包含1270个腹腔镜训练任务的立体视频记录，带有结构化技能评分和错误标签，用于支持基于深度学习的腹腔镜手术技能自动评估研究。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术训练需要大量练习，深度学习在视频技能评估方面有潜力，但受限于标注数据集的规模不足。

Method: 收集1270个立体视频记录，涵盖四种基本腹腔镜训练任务，由三位独立评分者提供结构化技能评分和任务特定错误标签，并提供预定义的数据分割。

Result: 建立了大规模标注数据集，提供了深度学习模型的基准结果，为未来研究提供参考比较点。

Conclusion: LASANA数据集填补了腹腔镜手术技能评估领域的数据空白，为开发更准确的深度学习模型提供了重要资源，有望推动腹腔镜手术培训的自动评估技术发展。
...
TL;DR: RoSE提出了一种新的单目法线估计范式，将法线估计重新定义为着色序列估计，利用图像到视频生成模型预测着色序列，然后通过最小二乘法转换为法线图，在真实数据集上达到state-of-the-art性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度模型的单目法线估计方法存在3D不对齐问题：虽然估计的法线图外观正确，但重建的表面无法与几何细节对齐。这是因为模型难以区分和重建法线图中表示的几何变化，因为底层几何的差异仅通过相对细微的颜色变化反映。

Method: 提出新范式：将法线估计重新表述为着色序列估计，因为着色序列对各种几何信息更敏感。基于此提出RoSE方法：利用图像到视频生成模型预测着色序列，然后通过求解普通最小二乘问题将着色序列转换为法线图。使用包含多样形状、材质和光照条件的合成数据集MultiShade进行训练。

Result: 实验证明RoSE在真实世界基准数据集上实现了最先进的物体单目法线估计性能。

Conclusion: 通过将法线估计重新定义为着色序列估计的新范式，RoSE有效解决了3D不对齐问题，证明了着色序列相比直接法线预测对几何信息更敏感，为单目法线估计提供了新的有效方法。
...
TL;DR: GeoFormer是一个基于Swin Transformer的开源框架，仅使用Sentinel-1/2影像和公开DEM数据，在100米网格上联合估计建筑物高度和占地面积，在54个城市评估中显著优于CNN基线模型，并具备良好的跨大陆迁移能力。


<details>
  <summary>Details</summary>
Motivation: 准确的三维城市数据对气候建模、灾害风险评估和城市规划至关重要，但现有方法依赖专有传感器或跨城市泛化能力差，数据稀缺。

Method: 提出GeoFormer框架，采用Swin Transformer架构，使用Sentinel-1/2多源遥感数据和公开DEM数据，通过地理区块划分策略确保训练集和测试集的空间独立性。

Result: 在54个多样化城市评估中，建筑物高度RMSE为3.19米，占地面积RMSE为0.05，比最强CNN基线分别提升7.5%和15.3%，跨大陆迁移时建筑物高度RMSE保持在3.5米以下。消融研究证实DEM对高度估计不可或缺，光学反射率优于SAR，但多源融合获得最佳精度。

Conclusion: GeoFormer提供了一个准确、开源的三维城市数据生成解决方案，所有代码、权重和全球产品均已公开发布，为解决城市三维数据稀缺问题提供了有效途径。
...
TL;DR: 提出基于非平衡最优传输(UOT)的配准感知匹配器，用于解决癌症患者纵向CT扫描中病灶演变评估的挑战，能够处理病灶出现、消失、合并和分裂等情况，无需重新训练或启发式规则。


<details>
  <summary>Details</summary>
Motivation: 评估癌症患者纵向CT扫描中的病灶演变对治疗反应评估至关重要，但建立跨时间可靠病灶对应关系仍然具有挑战性。传统的二分匹配器依赖几何邻近性，在病灶出现、消失、合并或分裂时表现不佳。

Method: 使用非平衡最优传输(UOT)方法，结合(1)尺寸归一化的几何信息、(2)变形场雅可比矩阵的局部配准可信度、(3)可选的斑块级外观一致性来计算传输成本。通过相对剪枝稀疏化传输计划，获得一对一匹配以及新出现、消失、合并和分裂的病灶。

Result: 在纵向CT数据上，该方法相比仅基于距离的基线方法，实现了持续更高的边缘检测精度和召回率，改善的病灶状态召回率，以及更优的病灶图组件F1分数。

Conclusion: 基于非平衡最优传输的配准感知匹配器能够有效解决纵向CT扫描中病灶对应关系建立的挑战，为癌症治疗反应评估提供了更可靠的病灶演变分析方法。
...
TL;DR: VersaViT是一个多任务视觉Transformer框架，通过轻量级任务头和多粒度监督优化MLLM视觉编码器，解决了其在密集预测任务中的不足，实现了同时支持语言介导推理和像素级理解的通用视觉骨干网络。


<details>
  <summary>Details</summary>
Motivation: 发现多模态大语言模型(MLLM)的视觉编码器虽然在高级语义对齐方面表现优异，但在密集预测任务(如语义分割、深度估计)中表现欠佳，需要开发能够同时处理语言推理和像素级视觉任务的通用视觉骨干网络。

Method: 提出VersaViT方法，采用多任务协作后训练框架，通过轻量级任务头和multi-granularity supervision来优化视觉骨干网络。

Result: 在多个下游任务上的广泛实验证明了该方法的有效性，能够产生既适合语言介导推理又适合像素级理解的通用视觉骨干网络。

Conclusion: VersaViT成功解决了MLLM视觉编码器在密集特征表示方面的缺陷，提供了一个全面的视觉Transformer解决方案，为多模态理解和密集预测任务提供了统一的视觉骨干网络。
...
TL;DR: 提出Hybrid Attention-Convolution (HAC)架构，结合Transformer和CNN，用于膀胱癌内窥镜检查中血管分割，解决膀胱变形和黏膜褶皱干扰问题，在BlaVeS数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌监测需要跟踪重复干预中的肿瘤位置，但膀胱的可变形和中空特性缺乏稳定标志物。内窥镜下可见血管可作为患者特异性"血管指纹"用于导航，但自动分割面临稀疏标签、伪影、连续变形和黏膜褶皱等挑战。

Method: 采用混合注意力-卷积架构：Transformer捕获全局血管拓扑先验，CNN学习残差细化图以精确恢复细血管细节。使用排除短小和末端分支的优化ground truth数据训练Transformer以优先考虑结构连通性。采用基于物理感知的预训练策略，在未标记数据上使用临床基础增强进行自监督学习。

Result: 在BlaVeS内窥镜视频帧数据集上，方法达到高准确率(0.94)、优异精确度(0.61)和clDice(0.66)，优于现有医学分割模型。成功抑制膀胱充盈和排空过程中动态出现的黏膜褶皱假阳性。

Conclusion: HAC方法提供了临床导航所需的可靠结构稳定性，能够有效应对膀胱内窥镜检查中的特定领域挑战。
...
TL;DR: 自动化德国烘焙食品剩余产品监测，通过弱监督学习和伪标签传播训练YOLOv11检测模型，仅使用图像级监督在非理想部署条件下超越全监督基线模型性能


<details>
  <summary>Details</summary>
Motivation: 德国烘焙食品保质期短，人工监测成本高且不准确。需要自动化解决方案，但烘焙食品种类多样导致全监督训练成本过高且难以扩展

Method: 1. 结合OWLv2和Grounding DINO定位与图像级监督进行弱监督训练；2. 使用Segment Anything 2进行伪标签传播，通过视频帧微调提升视角鲁棒性；3. 选择YOLOv11进行检测任务训练

Result: 仅使用图像级监督的模型达到0.91 mAP，伪标签微调在非理想部署条件下提升性能19.3%，组合方法在非理想条件下超越全监督基线模型

Conclusion: 提出的弱监督和伪标签训练工作流程有效解决了专业领域标注数据稀缺的问题，为计算机视觉在工业部署中的专业化任务提供了可行解决方案
...
TL;DR: 提出了一个基于耦合扩散模型的语义分解框架，将语义分解构建为逆问题，通过重建驱动的引导项耦合扩散过程，并引入新颖的迭代采样方案，在合成语义分解任务中优于谐振子网络。


<details>
  <summary>Details</summary>
Motivation: 视觉场景通常由潜在因子组成，有效的识别、推理和编辑需要分解这些组合表示。谐振子网络作为耦合Hopfield网络被提出用于分解绑定表示，而最近研究表明Hopfield网络与扩散模型存在显著相似性，这启发了使用耦合扩散推理进行语义分解的研究。

Method: 将语义分解构建为逆问题，使用重建驱动的引导项耦合扩散过程，该引导项鼓励因子估计的组合与绑定向量匹配。同时引入新颖的迭代采样方案来提高模型性能。

Result: 经验证明，该耦合推理框架在一系列合成语义分解任务中优于谐振子网络。此外，研究表明基于注意力的谐振子网络是该框架的一个特例。

Conclusion: 提出的耦合扩散模型框架为语义分解问题提供了有效的解决方案，不仅性能优于现有方法，还揭示了谐振子网络与扩散模型之间的理论联系，为组合表示学习提供了新的研究方向。
...
TL;DR: 比较两种自动染色分类方法：多实例学习(MIL)流程和轻量级缩略图方法。MIL在内部数据上表现最佳(F1:0.941)，但缩略图方法在外部数据上泛化更好(F1:0.843)，且处理速度快300倍。


<details>
  <summary>Details</summary>
Motivation: 病理学中多种特殊染色对诊断至关重要，保持准确的染色元数据对临床档案质量控制和计算病理学数据集完整性极为重要。

Method: 评估两种方法：1)多实例学习(MIL)全切片图像处理流程；2)提出的轻量级缩略图分类方法，覆盖14种常用特殊染色及标准/冰冻切片H&E染色。

Result: 内部测试：MIL性能最高(16类macro F1:0.941)；缩略图方法保持竞争力(0.897)。外部TCGA数据：缩略图模型泛化最佳(weighted F1:0.843 vs MIL 0.807)。缩略图方法处理速度比MIL快两个数量级(5.635 vs 0.018 slides/s)。
...
TL;DR: Faster-GS通过整合先前3DGS研究中最有效的策略并加入新优化，实现了5倍训练加速同时保持视觉质量，为3DGS优化建立了新的高效基准，并可扩展到4D高斯重建。


<details>
  <summary>Details</summary>
Motivation: 当前3DGS研究存在实现级改进与算法修改混杂、性能与质量权衡的问题，导致研究碎片化难以公平比较，需要系统整合和优化。

Method: 整合先前3DGS研究中最有效和广泛适用的策略，加入多个新颖优化，研究数值稳定性、高斯截断和梯度近似等未充分探索的方面。

Result: 在综合基准测试中实现高达5倍的训练加速，同时保持视觉质量，并能有效应用于4D非刚性场景优化。

Conclusion: Faster-GS提供了一个经过严格优化的算法，建立了成本效益和资源效率更高的3DGS优化基准，展示了优化策略在4D重建中的可扩展性。
...
TL;DR: 提出一种仅使用相机图像和已知目标几何形状的3D姿态认证估计方法，通过可达性分析和形式化神经网络验证来边界计算姿态，确保安全关键领域的最坏情况安全保证。


<details>
  <summary>Details</summary>
Motivation: 在安全关键系统中，传统姿态估计方法提供的粗略估计不足以确保最坏情况下的安全保证，且外部服务（如GPS）可能不可信，需要一种可靠的形式化验证方法。

Method: 结合可达性分析和形式化神经网络验证技术，仅利用相机图像和已知目标几何形状，通过边界计算来形式化地确定3D姿态估计。

Result: 实验表明该方法在合成和真实世界实验中都能高效、准确地定位智能体。

Conclusion: 该方法为安全关键系统提供了一种可靠的形式化姿态估计解决方案，能够在最坏情况下保证安全，且不依赖外部不可信服务。
...
TL;DR: Fake-HR1是一个大规模混合推理模型，通过自适应决策机制在图像伪造检测任务中智能选择是否进行推理，既保持检测精度又显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有CoT推理方法在图像伪造检测中虽然有效，但对于明显伪造的图像会产生不必要的冗长推理，导致资源消耗和延迟增加，需要一种自适应决策机制来优化推理效率。

Method: 提出两阶段训练框架：1）混合微调（HFT）进行冷启动初始化；2）在线强化学习与混合推理分组策略优化（HGRPO）来隐式学习选择合适推理模式的时机。

Result: 实验结果表明，Fake-HR1能够自适应地对不同类型查询执行推理，在推理能力和生成检测性能上均超越现有LLMs，同时显著提高响应效率。

Conclusion: Fake-HR1成功解决了图像伪造检测中推理效率与精度平衡的问题，通过自适应推理决策机制实现了资源消耗与检测性能的最优权衡，为生成内容检测任务提供了高效解决方案。
...
TL;DR: 研究表明，即使经过颅骨剥离处理的脑部MRI图像仍包含独特特征，可以通过标准预处理和图像相似度计算实现跨数据库的个体匹配，准确率接近完美，这揭示了医疗数据共享中的隐私风险。


<details>
  <summary>Details</summary>
Motivation: 当前监管框架要求评估医疗数据共享的隐私风险，但现有研究主要依赖基于训练或计算密集型方法。本研究旨在证明使用标准预处理和图像相似度计算即可实现颅骨剥离MRI的个体匹配，为制定前瞻性数据共享政策提供依据。

Method: 采用标准预处理流程处理颅骨剥离的T1加权MRI图像，然后进行图像相似度计算，在不同时间间隔、扫描仪类型、空间分辨率和采集协议条件下测试跨数据库匹配能力。

Result: 实现了近乎完美的个体匹配准确率，即使存在潜在认知衰退的情况下，仍能成功匹配来自不同数据库的MRI样本，证明了该方法在不同条件下的鲁棒性。

Conclusion: 脑部MRI图像即使在颅骨剥离后仍包含可识别个体的独特特征，现有的监管框架需要重新评估隐私保护措施，本研究结果为制定更严格的医疗数据共享政策提供了重要科学依据。
...
TL;DR: 提出一种用于实例分割的保形预测算法，能够为像素坐标查询生成自适应置信集，提供预测掩码与真实实例掩码IoU满足特定阈值的概率保证。


<details>
  <summary>Details</summary>
Motivation: 当前实例分割模型缺乏原则性的不确定性量化，输出未校准且无法保证预测掩码接近真实值，需要解决这一局限性。

Method: 开发保形预测算法，基于图像和像素坐标查询生成实例预测的置信集，提供渐进和有限样本的理论保证。

Result: 在农业地块划分、细胞分割和车辆检测等应用中，预测集大小随查询难度变化，达到目标覆盖概率，优于Learn Then Test、Conformal Risk Control和形态学膨胀等基线方法。

Conclusion: 该方法为实例分割提供了可靠的不确定性量化工具，具有理论保证和实际应用价值，能够根据查询难度自适应调整置信集大小。
...
TL;DR: 提出了一种时空注意力机制(STA)，将transformer的自注意力扩展到多帧处理，显著提升视频语义分割的时序一致性和准确性，在多个数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有深度神经网络在语义分割中独立处理视频帧，未能利用时序一致性，这限制了在动态场景中的准确性和稳定性提升。

Method: 扩展transformer的自注意力机制为时空注意力(STA)，处理时空特征序列，保持计算效率且对现有架构改动最小。

Result: 在Cityscapes和BDD100k数据集上，时序一致性指标提升9.20个百分点，mIoU提升最高达1.76个百分点。

Conclusion: STA是一种有效的架构增强方法，适用于各种transformer架构，为视频语义分割应用提供了显著性能提升。
...
TL;DR: Forensim是一个基于注意力机制的图像伪造检测框架，能够同时定位被篡改区域和源区域，通过状态空间模型和区域块注意力模块实现端到端训练，在标准基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖伪影线索检测伪造区域，在抗议图像等场景中可能误导解读，需要联合定位源区域和目标区域来理解上下文。

Method: 提出视觉状态空间模型，利用归一化注意力图识别内部相似性，配合基于区域的块注意力模块区分篡改区域，支持拼接和复制-移动伪造检测的统一架构。

Result: 在标准基准测试中达到最先进性能，同时发布了新的复制-移动伪造数据集CMFD-Anything以解决现有数据集局限性。

Conclusion: Forensim框架通过联合源-目标定位和注意力机制，有效解决了图像伪造检测中的上下文理解问题，为图像取证提供了更全面的解决方案。
...
TL;DR: 4RC是一个统一的单目视频4D重建前馈框架，通过编码一次、任意时空查询的新范式，联合学习密集场景几何和运动动态的完整4D表示


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将运动与几何解耦，或仅产生稀疏轨迹或两视图场景流等有限4D属性，需要一种能联合捕获密集几何和运动动态的整体4D表示方法

Method: 引入编码一次、任意时空查询的transformer架构：编码器将整个视频编码为紧凑时空潜在空间，条件解码器可从中高效查询任意目标时间戳的3D几何和运动；采用最小分解形式表示每视图4D属性，分解为基础几何和时间相关相对运动

Result: 在广泛的4D重建任务中优于现有和同期方法

Conclusion: 4RC框架通过统一的编码-解码架构成功实现了从单目视频中学习完整4D表示，为密集几何和运动动态的联合建模提供了有效解决方案
...
TL;DR: SCD是一种新的因果扩散架构，通过因果Transformer编码器解耦每帧一次的时间推理，使用轻量级扩散解码器进行多步逐帧渲染，在保持生成质量的同时显著提升吞吐量和延迟性能。


<details>
  <summary>Details</summary>
Motivation: 现有因果扩散模型将时间推理与迭代去噪过程纠缠在一起，存在计算冗余。研究发现早期层在去噪步骤间特征高度相似，深层主要执行帧内渲染，表明因果推理可与多步去噪过程分离。

Method: 提出可分离因果扩散(SCD)架构：使用因果Transformer编码器进行每帧一次的时间推理，轻量级扩散解码器处理多步逐帧渲染，减少冗余计算。

Result: 在合成和真实基准测试的预训练和后训练任务中，SCD在匹配或超越强因果扩散基线生成质量的同时，显著提高了吞吐量和每帧延迟。

Conclusion: 因果推理可与多步去噪过程有效分离，SCD架构通过解耦时间推理和帧渲染实现了计算效率的显著提升，为高效因果生成模型提供了新方向。
...
TL;DR: VideoWorld 2扩展了VideoWorld，首次研究从原始真实世界视频中直接学习可迁移知识。通过动态增强的潜在动态模型(dLDM)将动作动态与视觉外观解耦，使用预训练视频扩散模型处理视觉外观，让dLDM专注于学习紧凑且有意义的任务相关动态。在手工制作任务上实现70%的成功率提升，在机器人操作任务中显著改善性能。
...
Method: 提出动态增强的潜在动态模型(dLDM)，将动作动态与视觉外观解耦：预训练视频扩散模型处理视觉外观建模，dLDM学习专注于任务相关动态的潜在编码，并通过自回归方式建模学习任务策略和长时程推理。

Result: 在真实世界手工制作任务上实现高达70%的任务成功率提升，生成连贯的长执行视频；在机器人操作中从Open-X数据集获取有效操作知识，显著提升CALVIN任务性能。

Conclusion: 该研究揭示了直接从原始视频学习可迁移世界知识的潜力，所有代码、数据和模型将开源以供进一步研究。
...
TL;DR: SeqΔ-REPA方法通过序列级控制效应对齐目标，从大规模无标签视频中学习结构化潜在动作空间，实现零样本动作迁移和高效控制接口适应


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作学习方法存在跨上下文迁移困难的问题，因为标准目标仅在单个片段内操作，缺乏跨上下文的动作语义对齐机制

Method: 引入SeqΔ-REPA序列级控制效应对齐目标，将集成潜在动作锚定到冻结自监督视频编码器的时间特征差异上，构建Olaf-World动作条件视频世界模型预训练流程

Result: 实验表明该方法学习到更具结构化的潜在动作空间，在零样本动作迁移和新控制接口的数据高效适应方面优于现有基线方法

Conclusion: 通过利用可观察的动作语义效应作为共享参考，该方法成功解决了潜在动作跨上下文迁移的问题，为大规模无监督动作学习提供了有效解决方案
...
TL;DR: 论文提出了ConsID-Gen框架，通过多视图辅助和双流编码器解决图像到视频生成中的外观漂移和几何失真问题，并建立了大规模数据集和评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有图像到视频生成方法存在外观漂移和几何失真问题，主要源于单视图2D观测的稀疏性和弱跨模态对齐。

Method: 构建ConsIDVid大规模数据集和评估基准；提出ConsID-Gen框架，使用未姿态辅助视图增强第一帧，通过双流视觉-几何编码器和文本-视觉连接器融合语义与结构线索。

Result: 在ConsIDVid-Bench上实验表明，ConsID-Gen在多项指标上优于Wan2.1和HunyuanVideo等领先模型，在身份保真度和时序一致性方面表现优异。

Conclusion: 该方法通过数据增强和模型架构创新有效解决了I2V生成中的一致性问题，为高质量视频生成提供了新思路。
...
TL;DR: IQARS是首个将多旋转平均问题转化为可在量子退火器上执行的局部二次非凸子问题的算法，通过量子隧穿和并行性提升解空间探索效率，在D-Wave退火器上比最佳经典方法Shonan精度提高约12%


<details>
  <summary>Details</summary>
Motivation: 传统L1-IRLS和Shonan等方法存在局部极小值敏感性和凸松弛依赖问题，无法准确保持旋转流形几何结构，在高噪声场景下精度受限

Method: 将MRA问题重构为一系列局部二次非凸子问题，通过二值化处理后可在量子退火器上执行，利用量子隧穿效应和硬件并行性

Result: 在当前规模有限的量子退火器上，IQARS相比性能最佳的经典方法Shonan实现了约12%的精度提升

Conclusion: IQARS展示了量子计算在3D视觉优化问题中的潜力，虽然当前量子硬件仍处早期阶段，但该方法为未来大规模量子计算应用奠定了基础
...
TL;DR: SAGE是一个智能代理框架，能够根据用户指定的具身任务自动生成仿真就绪的3D环境，通过多生成器和评估器确保语义合理性、视觉真实性和物理稳定性。


<details>
  <summary>Details</summary>
Motivation: 真实世界数据收集成本高且不安全，现有场景生成系统依赖规则化或任务特定流程，产生伪影和物理无效场景，需要可扩展、真实且仿真就绪的3D环境。

Method: 采用代理框架，结合布局和对象组合的多生成器，通过评估器评估语义合理性、视觉真实性和物理稳定性，通过迭代推理和自适应工具选择自我优化场景。

Result: 生成的环境真实、多样，可直接在现代仿真器中部署用于策略训练。纯基于此数据的策略训练显示出明显的扩展趋势，并能泛化到未见过的对象和布局。

Conclusion: SAGE框架展示了仿真驱动扩展在具身AI中的潜力，能够大规模生成高质量的仿真就绪环境，支持策略训练和泛化能力。
...
### [98] [Overview of PAN 2026: Voight-Kampff Generative AI Detection, Text Watermarking, Multi-Author Writing Style Analysis, Generative Plagiarism Detection, and Reasoning Trajectory Detection](https://arxiv.org/abs/2602.09147)
...
TL;DR: PAN 2026研讨会聚焦计算风格学与文本取证，包含5个任务：生成式AI检测、文本水印、多作者风格分析、生成式抄袭检测和推理轨迹检测，延续Docker容器提交方式。


<details>
  <summary>Details</summary>
Motivation: 推动计算风格学和文本取证领域的发展，通过客观可复现的评估方法来应对生成式AI带来的新挑战，包括检测AI生成内容、验证文本水印鲁棒性等多维度问题。

Method: 采用基于Docker容器的软件提交方式，通过TIRA实验平台进行标准化评估。设置五个核心任务：Voight-Kampff生成式AI检测、文本水印鲁棒性测试、多作者写作风格分析、生成式抄袭检测、推理轨迹检测。

Result: 自2012年以来已接收超过1,100次Docker容器提交，建立了成熟的实验评估体系。2026年新增文本水印和推理轨迹检测两个新任务，扩展了检测范围。

Conclusion: PAN研讨会通过系统化的任务设计和标准化的评估平台，持续推动文本取证技术发展，特别是在应对生成式AI技术挑战方面提供了重要的研究框架和基准测试环境。
...
TL;DR: 提出了一种基于话语分析的包容性分析框架，用于实时评估人机协作学习环境中的动态包容性过程，包含参与公平、情感氛围和认知公平三个维度。


<details>
  <summary>Details</summary>
Motivation: 当前AI与教育领域的包容性评估主要依赖粗糙的样本描述或事后自我报告，无法捕捉协作问题解决过程中实时形成的包容性动态。

Method: 开发了包含三个互补维度（参与公平、情感氛围、认知公平）的包容性分析框架，使用可扩展的交互级测量方法，通过模拟对话和人机协作实验数据进行验证。

Result: 该框架能够揭示传统聚合或事后评估无法发现的参与模式、关系动态和观点采纳模式，使包容性过程分析可视化。

Conclusion: 这项工作代表了向过程导向的包容性测量方法迈出的重要一步，为人机协作学习环境提供了新的分析工具。
...
### [100] [Effective Reasoning Chains Reduce Intrinsic Dimensionality](https://arxiv.org/abs/2602.09276)
*Archiki Prasad,Mandar Joshi,Kenton Lee,Mohit Bansal,Peter Shaw*

Main category: cs.CL

TL;DR: 本研究提出内在维度作为量化推理链有效性的新指标，发现有效的推理策略能降低任务的内在维度，从而提升模型在分布内和分布外数据的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 尽管思维链推理显著提升了语言模型在复杂推理任务上的表现，但其促进泛化的具体机制仍不明确。当前解释主要关注测试时计算或结构指导，但缺乏量化这些因素与泛化能力之间关系的统一指标。

Method: 通过固定模型架构并改变任务表述（使用不同推理策略），采用内在维度作为量化指标——即达到特定准确率阈值所需的最小模型维度数。在GSM8K数据集上使用Gemma-3 1B和4B模型进行验证。

Result: 发现推理策略的有效性与任务内在维度呈强负相关关系：更有效的推理策略能显著降低任务的内在维度。这种关系在分布内和分布外数据上都保持一致。

Conclusion: 有效的推理链通过使用更少的参数更好地压缩任务来促进学习，内在维度为分析推理过程提供了一个新的量化度量标准，有助于理解不同推理策略的泛化机制。
...
### [101] [Don't Shoot The Breeze: Topic Continuity Model Using Nonlinear Naive Bayes With Attention](https://arxiv.org/abs/2602.09312)
*Shu-Ting Pi,Pradeep Bagavan,Yejia Li,Disha,Qun Liu*

Main category: cs.CL

TL;DR: 提出基于朴素贝叶斯和注意力机制的主题连续性模型，用于评估LLM对话响应是否保持主题连贯性，具有线性时间复杂度和良好可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM作为聊天机器人时出现的主题突然转换问题，避免用户体验不佳和计算资源浪费。

Method: 将自然语言理解模型扩展为量化形式，采用朴素贝叶斯方法，并引入注意力机制和对数非线性来增强主题连续性捕捉能力。

Result: 模型在实验中持续优于传统方法，特别擅长处理长而复杂的对话，具有线性时间复杂度和任意长度对话处理能力。

Conclusion: 该模型为LLM的责任性和可解释性使用提供了技术保障，能够有效维持对话主题的连贯性。
...
### [102] [Beyond Uniform Credit: Causal Credit Assignment for Policy Optimization](https://arxiv.org/abs/2602.09331)
*Mykola Khandoga,Rui Yuan,Vinay Kumar Sankarapu*

Main category: cs.CL

TL;DR: 提出反事实重要性加权方法，通过掩蔽推理片段、测量答案概率下降来调整策略梯度更新权重，使关键计算步骤获得更高权重，在GSM8K数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有策略梯度方法（如GRPO和DAPO）对所有生成token分配均匀信用，导致填充短语与关键计算步骤获得相同的梯度更新，这显然不合理。

Method: 反事实重要性加权：掩蔽推理片段，测量答案概率下降程度，根据概率下降幅度在策略梯度更新中相应上调token权重。无需辅助模型或外部标注，直接从策略模型自身的概率变化估计重要性。

Result: 在Qwen和Llama家族的三个模型上，GSM8K实验显示相比均匀基线方法有持续改进，收敛到相同准确率的速度更快。反转重要性信号会损害性能，证实方法捕捉到了真实的因果结构而非噪声。分析显示方法正确地将计算步骤优先级置于脚手架文本之上。

Conclusion: 该方法为后续研究奠定了反事实重要性加权的基础，但并非完整解决方案，需要进一步研究完善。
...
### [103] [FM SO.P: A Progressive Task Mixture Framework with Automatic Evaluation for Cross-Domain SOP Understanding](https://arxiv.org/abs/2602.09336)
*Siyuan Huang,Ziyu Wang,Chao Pan,Han Zhao*

Main category: cs.CL

TL;DR: FM SO.P是一个专门用于理解和处理标准操作流程(SOP)的模型，通过渐进式任务混合和自动多智能体评估系统，在七个不同领域实现了优异的性能表现，32B模型达到48.3%通过率，7B开源模型性能与72B基准模型相当。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在企业标准操作流程(SOP)理解和跨领域泛化方面存在困难，因为联合训练无法区分SOP所需的术语精确性、顺序性和约束推理等关键能力。

Method: 1. 渐进式任务混合：分阶段构建能力，包括概念消歧(术语精确性)、动作序列理解(程序正确性)、场景感知图推理(条件逻辑)；2. 自动多智能体评估系统：包含三个智能体，自适应生成评分标准、分层测试集和评分，适应不同领域需求。

Result: 在SOPBench七个领域(Bank、DMV、Healthcare、Market、University、Library、Hotel)评估中，32B模型达到48.3%通过率，7B开源模型达到34.3%通过率，与Qwen-2.5-72B-Instruct基准(34.4%)性能相当但参数量减少10倍。

Conclusion: FM SO.P通过创新的渐进式能力构建方法和自适应评估系统，有效解决了SOP理解和跨领域泛化的挑战，在保持高性能的同时显著降低了模型规模需求。
...
### [104] [Understanding Risk and Dependency in AI Chatbot Use from User Discourse](https://arxiv.org/abs/2602.09339)
*Jianfeng Zhu,Karin G. Coifman,Ruoming Jin*

Main category: cs.CL

TL;DR: 对2023-2025年Reddit社区中AI相关风险和成瘾讨论的大规模计算主题分析，识别出14个主题类别和5个高阶体验维度，揭示了AI相关心理风险的真实用户体验。


<details>
  <summary>Details</summary>
Motivation: 生成式AI系统日益融入日常生活，但对其心理风险如何产生、被体验和被用户调节的实证理解仍然有限，需要从真实用户话语中获取实证证据。

Method: 采用多智能体LLM辅助的主题分析方法，基于Braun和 Clarke的反身性框架，分析r/AIDangers和r/ChatbotAddiction两个Reddit社区的帖子，并使用BERT情感分类器进行情感标注和可视化分析。

Result: 识别出5个经验性AI相关心理风险体验维度，其中自我调节困难最为普遍，恐惧情绪集中在自主性、控制力和技术风险相关担忧中。
...
### [105] [Digital Linguistic Bias in Spanish: Evidence from Lexical Variation in LLMs](https://arxiv.org/abs/2602.09346)
*Yoshifumi Kawasaki*

Main category: cs.CL

TL;DR: 本研究评估大型语言模型对西班牙语地理词汇变异的识别能力，发现模型能较好识别西班牙、赤道几内亚、墨西哥和中美洲、拉普拉塔河地区的词汇变异，但对智利变体识别困难，且表现差异与数字资源数量无关。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型如何捕捉西班牙语这一具有显著地区变异的语言的地理词汇差异，探讨LLMs作为虚拟信息提供者的方言知识表征能力。

Method: 将LLMs视为虚拟信息提供者，使用是-否问题和多项选择题两种调查式问题格式，基于专家策划的大规模西班牙语词汇变异数据库，覆盖21个西班牙语国家的900多个词汇项，在国家层面和方言区域层面进行评估。

Result: 结果显示LLMs在表征西班牙语变体方面存在系统性差异：对西班牙、赤道几内亚、墨西哥和中美洲、拉普拉塔河地区的词汇变异识别更准确，智利变体识别特别困难；国家层面数字资源数量差异不能解释这些表现模式。

Conclusion: 本研究通过精细的大规模地理词汇变异评估，推进了对LLMs方言知识的实证理解，为西班牙语数字语言偏见讨论提供了新证据，表明数据量以外的因素影响LLMs中的方言表征。
...
### [106] [Unsupervised Cross-Lingual Part-of-Speech Tagging with Monolingual Corpora Only](https://arxiv.org/abs/2602.09366)
*Jianyu Zheng*
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言词性标注中平行语料稀缺的问题，现有基于词对齐的POS标注方法严重依赖平行语料，而许多低资源语言缺乏此类资源

Method: 使用无监督神经机器翻译系统将高资源语言句子翻译成低资源语言，构建伪平行句对；基于词对齐进行标准投影流程训练目标语言POS标注器；提出多源投影技术校准目标端投影的POS标签

Result: 在28个语言对（4个源语言和7个目标语言）上评估，性能与使用平行句对的基线方法相当，某些语言甚至更优；多源投影技术带来平均1.3%的性能提升

Conclusion: 该方法成功解决了低资源语言POS标注中平行语料依赖问题，仅使用单语语料即可实现与使用平行语料方法相当的性能，多源投影技术进一步提升了标注效果
...
### [107] [AgentSkiller: Scaling Generalist Agent Intelligence through Semantically Integrated Cross-Domain Data Synthesis](https://arxiv.org/abs/2602.09372)
*Zexu Sun,Bokai Ji,Hengyi Cai,Shuaiqiang Wang,Lei Wang,Guangxia Li,Xu Chen*

Main category: cs.CL

TL;DR: AgentSkiller是一个全自动框架，通过DAG架构和跨域融合机制生成高质量、长视野的多轮交互数据，显著提升大语言模型在工具调用方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在隐私限制或缺乏多样性，难以产生规模化能力所需的高质量长视野数据，制约了大语言模型代理的通用智能发展。

Method: 采用基于DAG的架构确保确定性和可恢复性，构建领域本体和人物中心实体图，通过服务蓝图定义工具接口，使用基于角色的模拟器生成查询并进行执行验证。

Result: 生成了约11,000个交互样本，实验显示在此数据集上训练的模型在函数调用方面相比基线有显著提升，尤其在更大参数规模下表现更优。

Conclusion: AgentSkiller框架能够有效合成真实、语义关联的跨领域交互数据，为大语言模型代理的能力扩展提供了可靠的数据生成解决方案。
...
### [108] [AfriNLLB: Efficient Translation Models for African Languages](https://arxiv.org/abs/2602.09373)
*Yasmin Moslem,Aman Kassahun Wassie,Amanuel Gizachew Abebe*

Main category: cs.CL

TL;DR: AfriNLLB是一个轻量级非洲语言翻译模型系列，通过层剪枝和量化技术压缩NLLB-200 600M模型，支持15种语言对（30个翻译方向）的高效翻译，在保持性能的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 旨在为资源受限环境提供高效的非洲语言翻译模型，解决非洲语言翻译模型部署的效率和资源消耗问题。

Method: 基于NLLB-200 600M模型，采用迭代层剪枝和量化技术进行压缩，在精心策划的非洲语言平行语料库上进行微调，并使用知识蒸馏技术从更大的教师模型学习。

Result: AfriNLLB模型在保持与基线相当性能的同时，实现了显著的速度提升，并发布了Transformers和CTranslate2两个版本供进一步微调和高效推理使用。

Conclusion: 该工作成功开发了高效的非洲语言翻译模型，为资源受限环境下的多语言翻译提供了可行解决方案，并公开了所有训练数据以促进进一步研究。
...
### [109] [BiasScope: Towards Automated Detection of Bias in LLM-as-a-Judge Evaluation](https://arxiv.org/abs/2602.09383)
*Peng Lai,Zhihao Ou,Yong Wang,Longyue Wang,Jian Yang,Yun Chen,Guanhua Chen*

Main category: cs.CL

TL;DR: BiasScope是一个LLM驱动的框架，用于自动发现模型评估中的潜在偏见，并基于此构建了更具挑战性的评估基准JudgeBench-Pro，揭示了当前LLM评估器存在的高错误率问题。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-Judge在研究和应用中被广泛采用，但其评估的鲁棒性和可靠性存在关键问题。现有研究主要关注已知偏见，缺乏对潜在未知偏见的自动化和系统性探索。

Method: 提出BiasScope框架，通过LLM驱动的方式自动、大规模地发现模型评估过程中可能出现的偏见，能够跨不同模型家族和规模进行偏见发现，并在JudgeBench数据集上验证了其通用性和有效性。

Result: 基于BiasScope构建了JudgeBench-Pro基准测试，即使是强大的LLM评估器在该基准上的错误率也超过50%，凸显了评估鲁棒性问题的严重性。

Conclusion: 该研究将偏见发现从依赖人工和预定义偏见列表的被动过程转变为主动、全面的自动化探索，强调了加强评估鲁棒性和进一步缓解潜在偏见的紧迫需求。
...
### [110] [Contractual Deepfakes: Can Large Language Models Generate Contracts?](https://arxiv.org/abs/2602.09384)
*Eliza Mik*

Main category: cs.CL

TL;DR: LLMs无法真正理解语言和法律含义，仅能生成统计模式上的文本近似，不适合用于合同起草等需要法律推理的专业任务


<details>
  <summary>Details</summary>
Motivation: 反驳将LLMs用于合同起草等法律任务的过度乐观观点，指出这种技术在处理具体交易情境和法律推理方面的根本局限性

Method: 通过分析LLMs的语言生成机制与法律合同起草的本质差异，论证统计模式预测与法律专业推理的根本区别

Result: LLMs生成的合同文件可能表面合理但实际上包含不一致条款，或虽可执行但不适合特定交易需求，存在严重适用性问题

Conclusion: LLMs并不构成对法律行业生存的威胁，不应被简单化地应用于需要专业法律推理的合同起草任务
...
### [111] [Effective vocabulary expanding of multilingual language models for extremely low-resource languages](https://arxiv.org/abs/2602.09388)
*Jianyu Zheng*

Main category: cs.CL

TL;DR: 提出一种通过词汇扩展和双语词典初始化来将多语言预训练模型扩展到未支持的低资源语言的方法，在词性标注和命名实体识别任务上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多语言预训练模型扩展方法主要关注继续预训练已支持的语言，而很少解决如何将模型扩展到先前未支持的低资源语言的问题。

Method: 使用目标语言语料扩展模型词汇表，筛选出偏向源语言的原始词汇子集，利用双语词典初始化扩展词汇的表示，然后基于扩展词汇表示继续使用目标语言语料进行预训练。

Result: 在词性标注和命名实体识别任务上分别比使用随机初始化扩展词汇的基线方法提升了0.54%和2.60%，且对训练语料选择具有高鲁棒性，源语言性能不会下降。

Conclusion: 该方法有效实现了多语言预训练模型向未支持低资源语言的扩展，通过双语词典初始化词汇表示显著提升了模型性能，同时保持了源语言的能力。
...
### [112] [Are Language Models Sensitive to Morally Irrelevant Distractors?](https://arxiv.org/abs/2602.09416)
*Andrew Shaw,Christina Hahn,Catherine Rasgaitis,Yash Mishra,Alisa Liu,Natasha Jaques,Yulia Tsvetkov,Amy X. Zhang*

Main category: cs.CL

TL;DR: 该研究评估了大型语言模型是否像人类一样对道德无关的情境因素敏感，通过向现有道德基准注入60种多模态道德干扰物，发现这些干扰物能使LLMs的道德判断产生超过30%的变化。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在高风险环境中的广泛应用，需要确保其行为符合人类价值观。现有道德基准假设LLMs具有稳定的道德偏好，但人类道德心理学研究表明人类道德判断易受情境因素影响，研究旨在检验LLMs是否表现出类似的认知道德偏见。

Method: 从现有心理学数据集中策划了60个多模态道德干扰物（情感性图像和叙事），这些干扰物与所呈现情境无道德相关性，将其注入现有道德基准中以测量对LLM响应的影响。

Result: 研究发现道德干扰物即使在低模糊度情境下也能使LLMs的道德判断产生超过30%的变化，表明LLMs对道德无关的情境因素具有敏感性。

Conclusion: 研究强调了LLMs需要更情境化的道德评估和更细致的认知道德建模，因为其道德判断易受无关情境因素影响，类似于人类的情况主义道德心理学现象。
...
### [113] [Breaking the Pre-Sampling Barrier: Activation-Informed Difficulty-Aware Self-Consistency](https://arxiv.org/abs/2602.09438)
*Taewoong Yoon,Geunyeong Jeong,Geon Park,Sihyeong Yeom,Harksoo Kim*

Main category: cs.CL

TL;DR: ACTSC是一种基于前馈网络激活信号的轻量级难度感知自一致性方法，通过神经元激活信息动态调整采样数量，无需额外token生成即可降低推理成本


<details>
  <summary>Details</summary>
Motivation: 传统自一致性方法需要大量采样导致推理成本高，而现有的难度自适应方法需要额外的模型调用和预采样来估计难度，计算开销大

Method: 利用前馈网络神经元激活中反映的内部难度信号构建轻量级难度估计探针，动态调整自一致性采样数量，无需额外token生成或模型调用

Result: 在五个基准测试上的实验表明，ACTSC能有效减少推理成本，同时保持与现有方法相当的准确性

Conclusion: ACTSC通过利用内部激活信号进行难度估计，实现了高效的自一致性推理，无需预采样即可应用于新数据集，显著降低了计算开销
...
### [114] [Evaluating Social Bias in RAG Systems: When External Context Helps and Reasoning Hurts](https://arxiv.org/abs/2602.09442)
*Shweta Parihar,Lu Cheng*

Main category: cs.CL

TL;DR: 本研究发现RAG架构通过引入外部知识源能够减少LLM的社会偏见，但结合Chain-of-Thought提示会增强偏见，揭示了偏见缓解与准确性之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中存在的社会偏见引发了严重的公平性问题，尽管RAG架构通过检索外部知识增强生成能力，但仍面临偏见挑战，需要系统评估RAG的偏见影响。

Method: 通过跨多个检索语料库、LLM模型和偏见评估数据集（涵盖13种以上偏见类型）的广泛实验，并整合Chain-of-Thought提示来分析模型的推理过程及其忠实性。

Result: 实验意外发现RAG能够减少偏见，外部上下文有助于抵消刻板印象驱动的预测；但CoT提示虽然提高准确性，却增加了整体偏见水平。

Conclusion: 外部知识检索有助于改善LLM的公平性，但需要开发偏见感知的推理框架来平衡准确性与偏见缓解之间的权衡关系。
...
### [115] [Conceptual Cultural Index: A Metric for Cultural Specificity via Relative Generality](https://arxiv.org/abs/2602.09444)
*Takumi Ohashi,Hitoshi Iyatomi*

Main category: cs.CL

TL;DR: 提出概念文化指数（CCI）来衡量句子层面的文化特异性，通过比较目标文化与其他文化的普遍性估计差异来量化文化特异性，在二元分类任务中比直接使用LLM评分效果更好。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多文化环境中的应用日益增多，但缺乏系统性的句子层面文化特异性评估方法。

Method: 定义CCI为句子在目标文化中的普遍性估计与在其他文化中平均普遍性估计的差异，通过控制比较设置来操作性地定义文化范围，并在400个句子（200个文化特异性句子和200个通用句子）上进行验证。

Result: CCI得分分布呈现预期模式：文化特异性句子得分较高，通用句子得分较低。在二元分类任务中，CCI比直接LLM评分表现更好，在目标文化专门化模型上的AUC提高了10个百分点以上。

Conclusion: CCI提供了一种可解释的句子层面文化特异性量化方法，能够有效区分文化特异性内容和通用内容，为多文化环境下的语言模型评估提供了实用工具。
...
### [116] [NOWJ @BioCreative IX ToxHabits: An Ensemble Deep Learning Approach for Detecting Substance Use and Contextual Information in Clinical Texts](https://arxiv.org/abs/2602.09469)
*Huu-Huy-Hoang Tran,Gia-Bao Duong,Quoc-Viet-Anh Tran,Thi-Hai-Yen Vuong,Hoang-Quynh Le*

Main category: cs.CL

TL;DR: NOWJ系统在BioCreative IX的ToxHabits共享任务中提出多输出集成方法，用于西班牙语临床文本中的毒物使用检测，在触发检测和参数检测方面取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 解决从非结构化电子健康记录中提取药物使用信息的挑战，同时克服大型语言模型在临床NLP应用中存在的信任、控制和效率问题。

Method: 采用多输出集成系统，结合BETO模型与CRF层进行序列标注，使用多样化训练策略和句子过滤技术以提高精度。

Result: 最佳运行结果：触发检测F1分数0.94、精确度0.97，参数检测F1分数0.91。

Conclusion: 所提出的方法在低资源、领域特定的西班牙语临床文本处理中表现出色，为毒物使用信息提取提供了有效的解决方案。
...
### [117] [Listen to the Layers: Mitigating Hallucinations with Inter-Layer Disagreement](https://arxiv.org/abs/2602.09486)
*Koduvayur Subbalakshmi,Sabbir Hossain Ujjal,Venkata Krishna Teja Mangichetty,Nastaran Jamalipour Soofi*

Main category: cs.CL

TL;DR: CoCoA解码器是一种无需训练的解码算法，通过分析LLM中间层的表征不稳定性来检测和减少幻觉，使用混淆和一致性指标惩罚不稳定输出，显著提升多个模型的事实正确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易产生流畅但事实错误的文本（幻觉现象），这降低了其在下游任务中的可靠性和实用性。研究发现生成文本的事实性与模型内部层间的表征不稳定性相关。

Method: 提出CoCoA解码算法：1）定义两个指标量化中间层表征不稳定性；2）对高混淆输出进行惩罚；3）提出自信息门控变体CoCoA-SIG，动态调节惩罚以针对高惊喜度不稳定生成。方法无需额外训练。

Result: 在问答、摘要和代码生成等多个任务上的广泛实验表明，CoCoA显著提高了Llama-3、Qwen-2.5、Mistral等多个模型家族的事实正确性。
...
### [118] [Where-to-Unmask: Ground-Truth-Guided Unmasking Order Learning for Masked Diffusion Language Models](https://arxiv.org/abs/2602.09501)
*Hikaru Asano,Tadashi Kozuno,Kuniaki Saito,Yukino Baba*

Main category: cs.CL

TL;DR: Gt-Margin方法通过真实标记的概率差值确定最优解掩顺序，训练监督式解掩规划器来提升掩码扩散语言模型的推理性能，无需修改基础模型。


<details>
  <summary>Details</summary>
Motivation: 解决MDLM推理时解掩顺序依赖启发式置信度或强化学习的高成本问题，寻求更有效的方法来确定最优解掩位置顺序。

Method: 提出Gt-Margin评分机制，基于真实标记与其最强替代标记的概率差值构建oracle解掩顺序；训练学习排序的监督解掩规划器来模仿该顺序。

Result: 使用oracle解掩顺序显著提升生成质量，特别是在逻辑推理基准上；监督规划器集成到标准MDLM采样中提高了推理准确性。

Conclusion: Gt-Margin提供了一种有效的解掩顺序优化方法，通过监督学习实现推理性能提升，为MDLM的推理能力增强提供了新途径。
...
### [119] [EcoGym: Evaluating LLMs for Long-Horizon Plan-and-Execute in Interactive Economies](https://arxiv.org/abs/2602.09514)
...
TL;DR: EcoGym是一个用于评估LLM智能体长期规划和决策能力的通用基准测试平台，包含三个经济环境场景，采用标准化接口和预算化动作设计，支持1000+步的长期评估，重点关注商业相关结果指标。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体评估框架存在局限性：多为片段式、领域特定、缺乏持续经济动态的充分基础，需要更通用的长期规划评估基准。

Method: 设计并实现了三个多样化经济环境（Vending、Freelance、Operation），采用统一决策流程和标准化接口，在部分可观测和随机性条件下进行长期（1000+步）预算化动作评估。

Result: 对11个主流LLM的测试显示：没有单一模型在所有三个场景中表现最优，模型在高层策略或动作执行效率方面存在显著次优性。

Conclusion: EcoGym作为一个开放可扩展的测试平台，为透明评估长期规划智能体和研究现实经济设置中的可控性-效用权衡提供了有效工具，揭示了LLM在长期经济决策中的系统性挑战。
...
### [120] [The CLEF-2026 CheckThat! Lab: Advancing Multilingual Fact-Checking](https://arxiv.org/abs/2602.09516)
...
TL;DR: CheckThat! 2026实验室重点关注验证流程的三个核心任务：科学网络声明的来源检索、数值和时间声明的事实核查、以及完整事实核查文章的生成，这些任务涉及多语言环境下的分类、检索和生成挑战。


<details>
  <summary>Details</summary>
Motivation: 该实验室旨在开发创新技术来对抗在线传播中的虚假信息和操纵行为，特别关注验证流程的扩展和完善，以应对多语言和多平台的复杂环境。

Method: 通过设置三个具体任务来推进验证流程：Task 1（科学声明来源检索）、Task 2（数值和时间声明事实核查，增加推理组件）、Task 3（生成完整事实核查文章），这些任务涵盖了分类、检索和生成等多个技术维度。

Result: 实验室提出了一个全面的验证流程框架，将传统的核查任务扩展到包括来源检索、数值推理和文章生成等更复杂的挑战，为多语言环境下的虚假信息检测提供了系统性的解决方案。

Conclusion: CheckThat! 2026实验室通过整合检索、推理和生成技术，构建了一个更加完善的多语言事实验证管道，为应对在线虚假信息的复杂挑战提供了重要的技术基础和评估框架。
...
### [121] [Knowledge Integration Decay in Search-Augmented Reasoning of Large Language Models](https://arxiv.org/abs/2602.09517)
*Sangwon Yu,Ik-hwan Kim,Donghun Kang,Bongkyu Hwang,Junhwa Choi,Suk-hoon Jung,Seungki Hong,Taehee Lee,Sungroh Yoon*

Main category: cs.CL

TL;DR: SAKE是一种无需训练的推理时策略，通过在推理过程首尾锚定检索知识来解决知识整合衰减问题，显著提升大语言模型在多跳问答和复杂推理任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 发现大语言模型在搜索增强推理中存在知识整合衰减问题，即随着推理链增长，模型越来越难以将检索到的证据整合到后续推理步骤中，即使相关信息可用也会限制性能。

Method: 提出Self-Anchored Knowledge Encoding方法，在推理过程的开头和结尾锚定检索到的知识，防止其被先前的上下文信息所掩盖，从而保持其语义完整性。

Result: 在多跳问答和复杂推理基准测试中的广泛实验表明，SAKE显著减轻了知识整合衰减问题并提高了性能。

Conclusion: SAKE为智能体大语言模型中的知识整合提供了一种轻量级但有效的解决方案，无需额外训练即可在推理时稳定知识利用。
...
### [122] [UniARM: Towards a Unified Autoregressive Reward Model for Multi-Objective Test-Time Alignment](https://arxiv.org/abs/2602.09538)
*Hongyan Xie,Yikun Ban,Ruiyu Fang,Zixuan Huang,Deqing Wang,Jianxin Li,Yitong Yao,Chao Wang,Shuangyong Song*

Main category: cs.CL

TL;DR: 提出了MoSLoRA和UniARM框架，通过共享特征提取和偏好调制模块解决多目标对齐中特征纠缠问题，实现单参数空间的多目标测试时对齐。


<details>
  <summary>Details</summary>
Motivation: 现有多目标对齐方法存在两个问题：一是为每个偏好目标独立训练ARMs忽略了偏好特征间的交互；二是使用独立特征提取模块会导致特征纠缠。这两种策略都会导致生成输出与用户偏好失配。

Method: 提出MoSLoRA方法：先通过偏好无关模块提取共享特征，再通过偏好调制模块对共享特征进行仿射变换。基于此构建UniARM框架，在单一参数空间中联合建模所有偏好维度。

Result: 该方法缓解了特征纠缠问题，能够在推理过程中精确控制偏好权衡，并在更大规模LLMs上验证了实用性。

Conclusion: MoSLoRA和UniARM提供了一个有效的多目标测试时对齐解决方案，通过共享参数空间和偏好调制机制实现了更好的对齐效果和实用性。
...
### [123] [Comprehensive Comparison of RAG Methods Across Multi-Domain Conversational QA](https://arxiv.org/abs/2602.09552)
*Klejda Alushi,Jan Strich,Chris Biemann,Martin Semmann*

Main category: cs.CL

TL;DR: 系统性评估多轮对话问答中检索增强生成方法，发现简单方法如重排序、混合BM25和HyDE优于复杂方法，性能表现高度依赖数据集特性和对话长度。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要评估单轮设置下的RAG方法，缺乏对多轮对话问答中RAG方法的系统比较，而对话历史、指代消解和用户意图变化使检索变得复杂。

Method: 在八个不同领域的对话问答数据集上，使用统一实验设置评估基础和高级RAG方法，采用检索质量和答案生成指标，分析跨对话轮次的性能变化。

Result: 鲁棒但简单的方法（重排序、混合BM25、HyDE）持续优于基础RAG，而一些高级技术不仅没有提升反而性能下降。数据集特性和对话长度强烈影响检索效果。
...
### [124] [Advancing Block Diffusion Language Models for Test-Time Scaling](https://arxiv.org/abs/2602.09555)
*Yi Lu,Deyang Kong,Jianing Wang,Linsen Guo,Xue Wang,Qi Guo,Tao Gui,Xuanjing Huang,Wei Ye,Shikun Zhang,Wei Wang*

Main category: cs.CL

TL;DR: 提出了BACD和TCCF的统一框架，通过自适应解码和块级生成优化，在测试时扩展中显著提升BDLM的推理速度和效果


<details>
  <summary>Details</summary>
Motivation: 现有BDLM在测试时扩展设置下探索有限，长链推理中存在解码速度与效果的平衡挑战

Method: BACD（有界自适应置信解码）动态调整去噪策略；TCCF（粗思考细批判）分配不同块大小；渐进块大小扩展缓解性能下降

Result: 在TDAR-8B上相比TraDo-8B实现2.26倍加速和11.2分的AIME24性能提升

Conclusion: 该框架是解锁BDLM在复杂推理任务中测试时扩展潜力的重要进展
...
### [125] [LEMUR: A Corpus for Robust Fine-Tuning of Multilingual Law Embedding Models for Retrieval](https://arxiv.org/abs/2602.09570)
*Narges Baba Ahmadi,Jan Strich,Martin Semmann,Chris Biemann*

Main category: cs.CL

TL;DR: LEMUR：基于欧盟环境法规的大规模多语言语料库，通过PDF文本提取质量评估和法律领域微调，显著提升多语言法律检索性能，特别是低资源语言效果显著


<details>
  <summary>Details</summary>
Motivation: 解决多语言法律环境下LLM检索不可靠、缺乏领域适配嵌入模型的问题，现有语料库不适合语义检索且PDF源文本提取噪声严重

Method: 从24,953份EUR-Lex PDF文档构建25语言语料库，使用Lexical Content Score评估PDF提取质量，基于对比目标在单语和双语设置下微调多语言嵌入模型

Result: 法律领域微调持续提升Top-k检索准确率，低资源语言改进尤其显著，跨语言评估显示改进可迁移到未见语言，表明微调主要增强语言无关的法律内容表示

Conclusion: LEMUR为多语言法律检索提供高质量数据集，法律领域微调能有效提升检索性能并具有跨语言泛化能力，为法律AI应用提供重要资源
...
### [126] [Aligning Tree-Search Policies with Fixed Token Budgets in Test-Time Scaling of LLMs](https://arxiv.org/abs/2602.09574)
*Sora Miyamoto,Daisuke Oba,Naoaki Okazaki*

Main category: cs.CL

TL;DR: BG-MCTS是一种预算引导的树搜索解码算法，通过动态调整搜索策略来适应剩余token预算，从广泛探索逐步转向精炼和答案完成，避免传统预算无关方法中的过分支或过早终止问题。


<details>
  <summary>Details</summary>
Motivation: 现有的树搜索解码方法主要将预算视为终止条件，而不是主动管理搜索过程，这会导致后期过度分支或提前终止，无法在固定token预算下实现最优性能。

Method: 提出预算引导MCTS算法，根据剩余token预算动态调整搜索策略：预算充足时进行广泛探索，预算减少时优先进行精炼和答案完成，并减少浅层节点的后期分支。

Result: 在MATH500和AIME24/25数据集上，使用开源权重LLM，BG-MCTS在不同预算设置下均优于预算无关的树搜索基线方法。

Conclusion: 预算引导的树搜索策略能够有效利用有限的token预算，显著提升解码性能，为实际部署中的固定预算约束提供了有效的解决方案。
...
### [127] [Context-Aware Counterfactual Data Augmentation for Gender Bias Mitigation in Language Models](https://arxiv.org/abs/2602.09590)
*Shweta Parihar,Liu Guangliang,Natalie Parde,Lu Cheng*

Main category: cs.CL

TL;DR: 提出Context-CDA方法，通过上下文增强的反事实数据增强和不确定性过滤，在保持语言建模能力的同时有效减少语言模型中的社会偏见。


<details>
  <summary>Details</summary>
Motivation: 传统反事实数据增强方法在减少语言模型偏见时可能导致语言建模能力下降，生成的数据与真实分布不符或忽略社会语境。

Method: 使用大型语言模型增强反事实数据的多样性和上下文相关性，通过不确定性过滤排除低质量生成数据，确保微调语料质量。

Result: 在性别偏见基准测试中，Context-CDA有效减轻偏见且不牺牲语言建模性能，通过分析下一个标记生成概率的分布变化提供社会偏见洞察。

Conclusion: Context-CDA是一种简单有效的方法，通过上下文增强和过滤机制，在去偏过程中保持模型的语言建模能力，为理解社会偏见提供了新视角。
...
### [128] [On the Optimal Reasoning Length for RL-Trained Language Models](https://arxiv.org/abs/2602.09591)
*Daisuke Nohara,Taishi Nakamura,Rio Yokota*

Main category: cs.CL

TL;DR: 本研究比较了不同长度控制方法对语言模型推理效率与性能的影响，发现长度惩罚可能阻碍推理能力，而适当调节的长度控制可提升已有强推理能力模型的效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能提升大语言模型的推理能力，但会导致思维链输出变长，增加训练和推理的计算成本。目前缺乏关于如何平衡效率与性能的最佳输出长度研究。

Method: 在Qwen3-1.7B Base和DeepSeek-R1-Distill-Qwen-1.5B两个模型上比较多种长度控制方法，并将先前工作扩展到RL训练策略中。

Result: 识别出两种失效模式：长输出增加分散性，短输出导致思考不足。长度惩罚可能阻碍推理获取，而适当调节的长度控制能提高具有强先验推理能力模型的效率。

Conclusion: 有效的长度控制需要在输出长度与推理质量之间找到平衡点，针对不同模型的推理能力特点采用适当的长度控制策略，以实现效率与性能的最优平衡。
...
### [129] [Learning from the Irrecoverable: Error-Localized Policy Optimization for Tool-Integrated LLM Reasoning](https://arxiv.org/abs/2602.09598)
*Qiao Liang,Yuke Zhu,Chao Ge,Lei Yang,Ying Shen,Bo Zheng,Sheng Guo*

Main category: cs.CL

TL;DR: ELPO通过二分搜索定位TIR轨迹中的首个不可恢复错误步骤，使用分层优势归因生成稳定学习信号，并通过自适应裁剪强化关键步骤的纠正更新，显著提升了Agentic RL在长时任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 在工具集成推理(TIR)中，仅基于结果的强化学习存在奖励稀疏、延迟和信用分配困难的问题，特别是在长时轨迹中早期不可恢复错误会直接影响任务成败，需要更精细的错误定位和信用分配机制。

Method: 提出错误定位策略优化(ELPO)：1) 在固定计算预算下通过二分搜索展开树定位首个不可恢复步骤；2) 使用分层优势归因将搜索树转换为稳定学习信号；3) 应用错误定位自适应裁剪强化关键步骤及其后续步骤的纠正更新。

Result: 在数学、科学问答和代码执行等TIR基准测试中，ELPO在相同采样预算下持续优于强Agentic RL基线，在Pass@K和Major@K扩展、展开排序质量和工具调用效率方面均有额外提升。

Conclusion: ELPO通过精确定位首个不可恢复错误并针对性强化学习，有效解决了TIR中长时轨迹的信用分配问题，为Agentic RL提供了更高效的训练范式。
...
### [130] [AlignTune: Modular Toolkit for Post-Training Alignment of Large Language Models](https://arxiv.org/abs/2602.09621)
...
TL;DR: AlignTune是一个模块化工具包，为LLM后训练对齐提供统一接口，解决现有工具碎片化、不可复现的问题，支持SFT和RLHF优化，具有可互换后端和可扩展奖励层。


<details>
  <summary>Details</summary>
Motivation: 当前LLM对齐研究存在后端干扰、奖励碎片化和流程不可复现的问题，需要统一工具来支持可控比较和可重复实验。

Method: 设计模块化架构，通过工厂模式隔离后端特定逻辑，提供TRL和Unsloth可互换后端，标准化配置系统，集成可扩展奖励层（基于规则和学习），并支持标准基准和自定义任务的评估。

Result: 实现了统一的对齐实验框架，能够进行受控的后端比较，确保实验流程的可重复性，为对齐研究提供了标准化的工具支持。

Conclusion: AlignTune成功解决了LLM对齐研究中的工具碎片化和不可复现性问题，为研究人员提供了统一的实验平台，促进了对齐方法的系统化比较和验证。
...
### [131] [MILE-RefHumEval: A Reference-Free, Multi-Independent LLM Framework for Human-Aligned Evaluation](https://arxiv.org/abs/2602.09624)
*Nalin Srun,Parisa Rastin,Guénaël Cabanes,Lydia Boudjeloud Assala*

Main category: cs.CL

TL;DR: MILE-RefHumEval是一个无需参考标注的无监督评估框架，通过集成多个独立提示的评估器来评估大语言模型，支持离散和连续评分，在多个任务上表现优异且计算开销低


<details>
  <summary>Details</summary>
Motivation: 为了解决大语言模型评估中需要真实标注和评估器协调的问题，开发一种无需参考、灵活且可扩展的评估方法

Method: 利用基于人类对齐模式的集成评估器，通过任务特定提示进行最佳候选选择、摘要、图像描述和对话等多种任务的评估

Result: 实验显示该方法与人类判断高度一致，优于现有方法，并显著减少计算开销

Conclusion: MILE-RefHumEval提供了一个高效、稳健且人类对齐的解决方案，适用于现实世界的大语言模型评估场景
...
### [132] [MATA: Multi-Agent Framework for Reliable and Flexible Table Question Answering](https://arxiv.org/abs/2602.09642)
*Sieun Hyeon,Jusang Oh,Sunghwan Steve Cho,Jaeyoung Do*

Main category: cs.CL

TL;DR: MATA是一个多智能体表格问答框架，通过结合小型语言模型工具和多样化推理路径，在保持高性能的同时显著减少对大语言模型的依赖，实现了高效、可扩展的表格理解解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在表格问答任务中面临可靠性、可扩展性和效率挑战，特别是在资源受限或隐私敏感环境中，需要开发更高效的解决方案。

Method: 提出多智能体框架MATA，使用小型语言模型构建工具集，生成多样化候选答案，通过算法优化减少昂贵的大语言模型调用，实现高效推理路径选择。

Result: 在10种不同大语言模型和两个难度不同的基准测试中，MATA达到了最先进的准确率，同时显著提高了推理效率，避免了过度的大语言模型推理开销。

Conclusion: 通过精心编排多个推理路径，MATA证明了在表格问答任务中可以实现可扩展且可靠的性能，为资源受限环境提供了有效的解决方案。
...
### [133] [Life Cycle-Aware Evaluation of Knowledge Distillation for Machine Translation: Environmental Impact and Translation Quality Trade-offs](https://arxiv.org/abs/2602.09691)
*Joseph Attieh,Timothee Mickus,Anne-Laure Ligozat,Aurélie Névéol,Jörg Tiedemann*

Main category: cs.CL

TL;DR: 知识蒸馏方法评估：同时考虑翻译质量和计算成本，使用碳足迹作为计算成本指标，发现不同部署规模下蒸馏开销和推理成本的主导地位不同，词级蒸馏通常比序列级蒸馏有更好的碳足迹-质量权衡。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏研究通常只报告学生模型的翻译质量，而忽略了计算复杂度，这使得在计算资源受限的情况下难以选择合适的蒸馏方法。

Method: 使用机器学习生命周期评估(MLCA)工具将计算成本表示为碳足迹，评估了代表性的知识蒸馏方法，考虑了运行时操作排放和硬件生产成本的摊销，覆盖蒸馏模型生命周期（教师训练、蒸馏和推理）的各个环节。

Result: 研究发现：(i)在小规模部署时蒸馏开销主导总碳足迹，(ii)在大规模部署时推理成本主导，使得知识蒸馏仅在超过任务相关的使用阈值时才有利，(iii)词级蒸馏通常比序列级蒸馏提供更有利的碳足迹-质量权衡。

Conclusion: 该研究提供了一个可重复的协议，用于在明确的质量和计算约束下选择知识蒸馏方法，为实际应用中的方法选择提供了实用指导。
...
### [134] [Maastricht University at AMIYA: Adapting LLMs for Dialectal Arabic using Fine-tuning and MBR Decoding](https://arxiv.org/abs/2602.09703)
*Abdulhai Alali,Abderrahmane Issam*

Main category: cs.CL

TL;DR: 使用LoRA微调、适配器合并和方言感知MBR解码来提升预训练大语言模型的方言性能，在叙利亚、摩洛哥和沙特阿拉伯方言上验证了方法有效性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然支持多语言，但方言变体因数据有限和语言变异而代表性不足，需要专门方法提升方言性能

Method: 采用低秩适应(LoRA)微调单语和英语方言平行数据，结合适配器合并和方言感知最小贝叶斯风险(MBR)解码技术

Result: 在叙利亚、摩洛哥和沙特阿拉伯阿拉伯方言上的实验表明，该方法在保持语义准确性的同时显著提高了方言保真度

Conclusion: 该方法提供了一个紧凑有效的框架，可用于鲁棒的阿拉伯方言生成，解决了方言数据稀缺和语言变异带来的挑战
...
### [135] [TraceMem: Weaving Narrative Memory Schemata from User Conversational Traces](https://arxiv.org/abs/2602.09712)
*Yiming Shu,Pei Liu,Tiange Zhang,Ruiyang Gao,Jun Ma,Chen Sun*

Main category: cs.CL

TL;DR: TraceMem是一个受认知启发的记忆框架，通过三阶段流水线构建结构化叙事记忆模式，在长对话交互中实现最先进性能，特别擅长多跳和时间推理。


<details>
  <summary>Details</summary>
Motivation: 现有LLM记忆系统将交互视为不连贯片段，无法捕捉对话流的叙事连贯性，而有限上下文窗口难以维持长期交互。

Method: 三阶段流水线：1)短时记忆处理-主题分割和语义提取；2)突触记忆巩固-将片段总结为情景记忆并提炼为用户特定痕迹；3)系统记忆巩固-通过分层聚类将痕迹组织成连贯的叙事线索，封装为结构化用户记忆卡。

Result: 在LoCoMo基准测试中达到最先进性能，在多跳和时间推理方面超越基线，证明其在深度叙事理解中的关键作用。

Conclusion: TraceMem通过构建连贯叙事有效解决了LLM长期交互的瓶颈问题，为记忆系统领域提供了新的认知启发框架和未来发展方向。
...
### [136] [Unsupervised Layer-Wise Dynamic Test Time Adaptation for LLMs](https://arxiv.org/abs/2602.09719)
*Longhuan Xu,Cunjian Chen,Feng Yin*

Main category: cs.CL

TL;DR: 提出了一种层级动态测试时适应框架，通过轻量级超网络预测每层每步的学习率乘数，解决无监督样本特定TTA中固定学习率导致的不稳定问题，显著提升了适应效果和生成质量。


<details>
  <summary>Details</summary>
Motivation: 无监督样本特定的测试时适应（TTA）虽然具有吸引力，但使用固定手工学习率的朴素方法会导致模型过拟合到提示特定统计量、偏离期望答案分布，最终降低生成质量。

Method: 提出层级动态测试时适应框架，仅更新LoRA参数，通过轻量级超网络预测每层每步的学习率乘数，实现细粒度控制。

Result: 在多个数据集和LLM上的实验一致表明，该方法通过学习适应步骤和Transformer层投影的有效缩放模式，显著增强了TTA，提高了稳定性并带来更好的性能。

Conclusion: 层级动态学习率调制是解决无监督样本特定TTA不稳定问题的有效方法，能够实现更稳定和高效的模型适应。
...
### [137] [AI-Assisted Scientific Assessment: A Case Study on Climate Change](https://arxiv.org/abs/2602.09723)
...
TL;DR: AI辅助科学评估系统在气候科学领域的应用研究，通过Gemini模型支持科学家协作评估AMOC稳定性问题，显著加速科研流程但需要专家深度参与和监督


<details>
  <summary>Details</summary>
Motivation: 当前AI co-scientists范式主要适用于可重复验证的任务，但对于需要理论证据共识建立的复杂科学问题缺乏支持，因此需要开发适合这类问题的AI协作评估系统

Method: 开发基于Gemini的AI协作环境，与13位气候科学家合作，在AMOC稳定性这一复杂主题上进行测试，通过104次修订循环分析79篇论文

Result: AI显著加速科研工作流程：46人时内完成全面文献综述，大部分AI生成内容被保留，AI帮助保持逻辑一致性和呈现质量，但专家贡献仍占报告一半以上且需要大量监督

Conclusion: AI能够有效加速科学工作流程，但在需要理论证据共识的复杂科学问题中，专家监督和深度参与仍是确保科学严谨性的关键要素，AI主要起到辅助而非替代作用
...
### [138] [Targum -- A Multilingual New Testament Translation Corpus](https://arxiv.org/abs/2602.09724)
*Maciej Rapacz,Aleksander Smywiński-Pohl*

Main category: cs.CL

TL;DR: 构建了一个包含657个新约译本的多语言语料库，其中352个是独特版本，在英语、法语、意大利语、波兰语和西班牙语五种语言中提供了前所未有的翻译深度，支持从微观到宏观的多层次翻译历史研究。


<details>
  <summary>Details</summary>
Motivation: 现有语料库在追求语言广度时往往忽略了翻译历史的深度，特别是圣经翻译的丰富历史传统，需要专门资源来支持定量翻译研究。

Method: 从12个在线圣经图书馆和一个现有语料库中聚合数据，包含657个新约译本，每个译本都经过手动元数据标注，包括标准化的作品标识符、特定版本和修订年份信息。

Result: 创建了首个支持灵活多层次分析的多语言圣经翻译语料库，包含英语(208个独特版本/396个总数)、法语(41/78)、意大利语(18/33)、波兰语(30/48)和西班牙语(55/102)的深度数据。

Conclusion: 该语料库为翻译历史的定量研究设立了新基准，使研究人员能够根据自身需求定义"独特性"，进行从KJV谱系等微观分析到宏观层面的去重研究。
...
### [139] [Improving Interpretability of Lexical Semantic Change with Neurobiological Features](https://arxiv.org/abs/2602.09760)
*Kohei Oda,Hiroya Takamura,Kiyoaki Shirai,Natthawut Kertkeidkachorn*

Main category: cs.CL

TL;DR: 提出一种将词嵌入语义空间映射到神经生物学特征空间的方法，通过可解释的原始特征维度来系统分析词汇语义演变，在保持高解释性的同时提升了LSC检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有词汇语义演变研究主要关注性能提升但缺乏可解释性，难以理解词义如何具体变化，需要增强LSC的可解释性以获得新见解。

Method: 将预训练语言模型获取的上下文词嵌入语义空间映射到神经生物学特征空间，每个维度对应词的原始特征，数值表示特征强度，实现人类可解释的LSC分析。

Result: 方法在LSC程度估计上优于多数现有方法，高解释性支持深入分析，发现了以往研究中被忽视的LSC类型，并能有效搜索特定类型的语义演变词汇。

Conclusion: 通过神经生物学特征空间映射实现了LSC的高解释性系统分析，不仅提升了性能还发现了新的语义演变模式，为词汇语义演变研究提供了新的分析工具和见解。
...
### [140] [Where Are We At with Automatic Speech Recognition for the Bambara Language?](https://arxiv.org/abs/2602.09785)
*Seydou Diallo,Yacouba Diarra,Mamadou K. Keita,Panga Azazia Kamaté,Adam Bouno Kampo,Aboubacar Ouattara*

Main category: cs.CL

TL;DR: 首个班巴拉语ASR标准化基准，基于1小时专业录制的马里宪法文本，评估37个模型发现当前性能远低于部署标准，最佳WER为46.76%，CER为13.00%，显示多语言预训练和模型缩放对低资源语言不足。


<details>
  <summary>Details</summary>
Motivation: 为班巴拉语这一低资源语言建立首个标准化语音识别评估基准，解决缺乏可靠评估标准的问题，促进该语言语音技术的发展。

Method: 使用1小时专业录制的马里宪法文本创建标准化基准，在接近最优的声学和语言条件下评估37个模型，包括班巴拉语专用系统和大型商业多语言模型。

Result: 最佳词错误率(WER)为46.76%，最佳字符错误率(CER)为13.00%，多个知名多语言模型WER超过100%，表明当前ASR性能远未达到部署标准。

Conclusion: 多语言预训练和模型缩放对低资源语言效果有限，需要针对性开发；该基准代表最理想情况，实际应用性能可能更差；提供公开基准和排行榜促进透明评估和未来研究。
...
### [141] [Decomposing Reasoning Efficiency in Large Language Models](https://arxiv.org/abs/2602.09805)
*Daniel Kaiser,Arnoldo Frigessi,Ali Ramezani-Kebrya,Benjamin Ricaud*

Main category: cs.CL

TL;DR: 提出了一个可选的追踪框架来分解LLM推理中的token效率，包含完成度、条件正确性和冗余度三个核心因素，在25个模型评估中发现准确性与token效率排名存在差异，效率差距主要由条件正确性驱动。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅关注最终准确性，掩盖了token在推理过程中的使用效率问题，无法识别token被浪费或有效利用的具体环节。

Method: 开发trace-optional框架，将token效率分解为：固定token预算下的完成度、完成后的条件正确性、冗余度。当有基准测试元数据时，进一步将冗余度分解为平均表达开销和耦合系数；有推理轨迹时加入确定性质量度量（基础性、重复性、提示复制）。

Result: 在CogniLoad数据集上评估25个模型，发现：准确性与token效率排名相关性仅为0.63；效率差距主要由条件正确性因素驱动；表达开销变化幅度达9倍且与模型规模关系较弱。

Conclusion: 该分解框架揭示了不同的效率瓶颈模式，为针对性的效率优化干预提供了指导方向，有助于更全面地评估LLM的推理效率。
...
### [142] [AnalyticsGPT: An LLM Workflow for Scientometric Question Answering](https://arxiv.org/abs/2602.09817)
*Khang Ly,Georgios Cheirmpos,Adrian Raudaschl,Christopher James,Seyed Amin Tabatabaei*

Main category: cs.CL

TL;DR: AnalyticsGPT是一个基于大语言模型的科学计量问答工作流，专门处理科学学领域的元科学问题，通过检索增强生成和智能体概念实现端到端系统，解决了学术实体识别和多维度数据检索的独特挑战。


<details>
  <summary>Details</summary>
Motivation: 科学计量问答作为科学学领域的下游任务面临独特挑战，需要识别学术实体并检索多维度科学计量指标，传统科学问答方法在此领域存在局限性，而大语言模型在任务分解和推理方面展现出巨大潜力。

Method: 采用检索增强生成和智能体概念的端到端顺序工作流，利用专有研究绩效评估平台作为检索数据库，结合命名实体识别和多维度数据检索技术，并采用专家评估和LLM-as-judges方法进行系统评估。

Result: 开发了完整的AnalyticsGPT系统，提供了有价值的见解，证明了大语言模型在特定下游任务中的有效性，系统代码和提示词已开源。

Conclusion: 该研究成功展示了LLMs在科学计量问答这一细分领域的应用潜力，为解决科学学领域的复杂问题提供了创新解决方案，为后续相关研究奠定了基础。
...
### [143] [Text summarization via global structure awareness](https://arxiv.org/abs/2602.09821)
...
TL;DR: GloSA-sum是一种基于拓扑数据分析的文本摘要方法，通过构建语义加权图和使用持久同调识别核心语义与逻辑结构，实现全局结构感知的高效摘要生成。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注模型改进和句子级剪枝，但忽视全局结构，导致连贯性破坏和下游性能下降；大语言模型虽然准确但资源消耗大。

Method: 构建语义加权图，利用持久同调识别核心语义和逻辑结构并存入保护池；设计拓扑引导的迭代策略，使用轻量代理指标近似句子重要性；提出分层策略整合段落级和全局摘要。

Result: 在多数据集上实验证明，GloSA-sum能减少冗余同时保持语义和逻辑完整性，在准确性和效率间取得平衡，并通过缩短上下文保留关键推理链来提升LLM下游任务性能。

Conclusion: GloSA-sum是首个通过拓扑数据分析实现全局结构感知的摘要方法，有效解决了现有方法在结构连贯性和效率方面的局限性，为长文本处理提供了新的解决方案。
...
### [144] [From FusHa to Folk: Exploring Cross-Lingual Transfer in Arabic Language Models](https://arxiv.org/abs/2602.09826)
*Abdulmuizz Khalak,Abderrahmane Issam,Gerasimos Spanakis*

Main category: cs.CL

TL;DR: 阿拉伯语言模型主要基于现代标准阿拉伯语(MSA)预训练，但实际应用中需要迁移到方言变体。研究发现跨方言迁移效果不均，与地理距离相关，且存在负向干扰现象。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语言模型主要在MSA上预训练，但实际网络交流使用多种方言，方言与MSA的相似度差异限制了模型的跨方言迁移能力。

Method: 通过3个NLP任务的探针分析和表征相似性研究，评估阿拉伯语言模型的跨方言迁移效果。

Result: 跨方言迁移可行但效果不均衡，地理邻近性部分解释迁移差异，发现支持所有方言的模型存在负向干扰。

Conclusion: 阿拉伯方言间的相似度值得质疑，跨方言迁移存在挑战，对阿拉伯语言模型的实用性提出担忧。
...
### [145] [LLM Reasoning Predicts When Models Are Right: Evidence from Coding Classroom Discourse](https://arxiv.org/abs/2602.09832)
*Bakhtawar Ahtisham,Kirk Vanacore,Zhuqian Zhou,Jinsook Lee,Rene F. Kizilcec*

Main category: cs.CL

TL;DR: LLM生成的理由可用于预测模型自身预测的正确性。通过分析教育对话数据，使用TF-IDF编码理由并训练监督分类器，随机森林达到F1分数0.83。研究发现正确预测使用因果语言，错误预测依赖认知模糊表达，为自动化教育对话分析提供了实用的质量控制方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在教育对话自动标注流程中缺乏可靠的错误检测机制，需要研究模型生成的理由是否能预测其自身预测的正确性。

Method: 分析30,300条教师话语，使用多个人工智能模型标注教学行为结构并生成理由。使用TF-IDF编码理由，评估五种监督分类器，并利用LIWC框架分析语言标记。

Result: 随机森林分类器F1分数0.83（召回率0.854），成功识别大部分错误预测。正确预测使用因果语言，错误预测依赖认知模糊表达和元认知表现。

Conclusion: 基于理由的错误检测为自动化教育对话分析提供了可扩展的质量控制方法，特定结构的专业检测器能进一步提高性能。
...
### [146] [How Do People Quantify Naturally: Evidence from Mandarin Picture Description](https://arxiv.org/abs/2602.09838)
*Yayun Zhang,Guanyi Chen,Fahime Same,Saad Mahamood,Tingting He*

Main category: cs.CL

TL;DR: 研究通过图片描述任务分析普通话使用者在自然条件下如何选择和使用量化表达，发现物体数量、生命性和输出模态系统性地影响量化行为。


<details>
  <summary>Details</summary>
Motivation: 量化是日常语言的基本组成部分，但人们对说话者在自然产出中如何决定是否以及如何进行量化知之甚少，需要研究无约束条件下的量化行为。

Method: 使用基于图片的诱发描述任务，让说话者自由描述包含多个物体的场景，不明确指示计数或量化，分析口语和书面语两种模态。

Result: 物体数量增加会降低量化的可能性和精确度；有生命指称和输出模态选择性地调节策略选择；系统性地揭示了量化行为的影响因素。

Conclusion: 本研究展示了如何在无约束产出条件下研究量化，为语言产出中的数量表达分析提供了自然主义数据集，揭示了量化行为受多重因素系统影响。
...
### [147] [SinFoS: A Parallel Dataset for Translating Sinhala Figures of Speech](https://arxiv.org/abs/2602.09866)
*Johan Sofalas,Dilushri Pavithra,Nevidu Jayatilleke,Ruvan Weerasinghe*

Main category: cs.CL

TL;DR: 该论文针对低资源语言僧伽罗语中的修辞手法翻译难题，构建了一个包含2344个文化注释的修辞语料库，开发了二元分类器（准确率92%），并评估了现有大语言模型在文化感知翻译方面的不足。


<details>
  <summary>Details</summary>
Motivation: 神经机器翻译在处理高资源语言的比喻表达时表现良好，但在处理像僧伽罗语这样的低资源语言时，由于数据有限而面临挑战，特别是文化相关的修辞手法翻译问题。

Method: 构建包含2344个僧伽罗语修辞手法的语料库，带有文化和跨语言注释；开发二元分类器区分两类修辞手法；评估现有大语言模型在该数据集上的表现。

Result: 分类器达到约92%的准确率；发现现有大语言模型在准确传达习语含义方面存在显著缺陷，经常难以正确处理文化相关的修辞表达。

Conclusion: 该数据集为低资源NLP和文化感知机器翻译研究提供了重要基准，揭示了当前LLM在文化敏感翻译任务上的局限性，需要进一步改进。
...
### [148] [Steer2Edit: From Activation Steering to Component-Level Editing](https://arxiv.org/abs/2602.09870)
*Chung-En Sun,Ge Yan,Zimo Wang,Tsui-Wei Weng*

Main category: cs.CL

TL;DR: Steer2Edit是一个无需训练的理论框架，将推理时的控制向量转换为诊断信号，通过组件级别的rank-1权重编辑实现更优的行为控制，避免了传统激活干预的全局修改问题。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型行为控制方法通过在推理时对隐藏表示应用固定的全局激活干预，虽然有效但往往导致属性-效用的不利权衡，忽视了行为由少量异质组件控制的事实。

Method: 提出Steer2Edit框架，将转向向量从推理时控制信号转换为组件级rank-1权重编辑的诊断信号，选择性地在注意力头和MLP神经元间重新分配行为影响，保持标准前向传播并与并行推理兼容。

Result: 在安全对齐、幻觉缓解和推理效率方面，Steer2Edit实现了更优的属性-效用权衡：在匹配下游性能时，安全性提升达17.2%，真实性提高9.8%，推理长度平均减少12.2%。

Conclusion: Steer2Edit通过将转向信号转换为可解释的无训练参数更新，为表示转向和权重编辑之间建立了原则性桥梁。
...
### [149] [The Devil Behind Moltbook: Anthropic Safety is Always Vanishing in Self-Evolving AI Societies](https://arxiv.org/abs/2602.09877)
...
TL;DR: 论文通过理论分析和实证研究表明，完全封闭的多智能体系统无法同时实现持续自我进化、完全隔离和安全不变性这三个目标（自我进化三难困境）。基于信息论框架，研究证明隔离的自我进化会导致统计盲点，造成安全对齐的不可逆退化，并提出了缓解方案。


<details>
  <summary>Details</summary>
Motivation: 探索多智能体系统在实现持续自我进化的同时保持安全对齐的可能性，解决所谓的"自我进化三难困境"问题。

Method: 采用信息论框架将安全性形式化为与人类价值分布的散度，通过理论证明和实证分析（包括开放智能体社区Moltbook和两个封闭自进化系统的实验）验证理论预测。

Result: 理论和实证结果一致表明，完全隔离的自我进化系统必然导致安全对齐的不可逆退化，存在统计盲点问题。

Conclusion: 研究确立了自进化AI社会的基本限制，需要从症状驱动的安全修补转向对内在动态风险的原则性理解，强调外部监督或新型安全保护机制的必要性。
...
### [150] [AmharicIR+Instr: A Two-Dataset Resource for Neural Retrieval and Instruction Tuning](https://arxiv.org/abs/2602.09914)
*Tilahun Yeshambel,Moncef Garouani,Josiane Mothe*

Main category: cs.CL

TL;DR: 发布了两个阿姆哈拉语数据集：1,091个检索-排序三元组和6,285个指令-响应对，支持神经检索排序和指令跟随文本生成研究，为低资源语言提供高质量监督数据和方法论。


<details>
  <summary>Details</summary>
Motivation: 阿姆哈拉语等低资源语言缺乏大规模高质量监督数据，限制了神经检索和生成模型的发展，需要构建专门的数据资源来支持相关研究。

Method: 检索数据集通过专家策划查询、网络查询和LLM辅助生成构建三元组；指令数据集使用多个LLM生成并通过人工审查修正语法、相关性、流畅性和事实合理性。

Result: 创建了1,091个手动验证的三元组数据集和6,285个指令-响应对数据集，涵盖多个领域和指令类型，提供标准化分割和格式(CSV,JSON,JSONL)。

Conclusion: 这些数据集为阿姆哈拉语检索、排序和生成建模提供了可复现的研究基础，其构建方法可推广到其他低资源语言，填补了该语言资源空白。
...
### [151] [LLMs Encode Their Failures: Predicting Success from Pre-Generation Activations](https://arxiv.org/abs/2602.09924)
*William Lugoloobi,Thomas Foster,William Bankes,Chris Russell*

Main category: cs.CL

TL;DR: 论文研究通过分析LLMs在生成前内部表征来预测模型在数学和编程任务上的成功概率，使用线性探针显著优于表面特征，发现模型编码的难度概念与人类不同，并利用此信号实现路由查询，在MATH数据集上减少70%推理成本同时超越最佳模型性能。


<details>
  <summary>Details</summary>
Motivation: 运行LLMs进行扩展推理成本高昂，但确定哪些输入真正需要额外计算仍然具有挑战性。研究旨在探索是否可以从模型生成前的内部表征中恢复其自身成功概率，并利用这一信号指导更高效的推理。

Method: 使用E2H-AMC数据集（提供人类和模型在相同问题上的表现），在生成前激活上训练线性探针来预测策略特定的成功概率，比较表面特征（如问题长度和TF-IDF）的性能差异。

Result: 线性探针在预测模型成功概率方面显著优于表面特征；发现模型编码的模型特定难度概念与人类难度不同，且这种差异随扩展推理增加；通过路由查询在模型池中分配问题，在MATH数据集上实现推理成本降低70%的同时超越最佳模型性能。

Conclusion: 模型内部表征即使与人类难度直觉存在差异，仍能实现实际的效率提升，为高效LLM推理提供了新途径。
...
### [152] [ATTNPO: Attention-Guided Process Supervision for Efficient Reasoning](https://arxiv.org/abs/2602.09953)
*Shuaiyi Nie,Siyu Ding,Wenyuan Zhang,Linhao Yu,Tianmeng Yang,Yao Chen,Tingwen Liu,Weichong Yin,Yu Sun,Hua Wu*

Main category: cs.CL

TL;DR: ATTNPO是一个基于注意力信号的轻量级过程监督RL框架，通过识别特殊注意力头来区分必要步骤和冗余步骤，有效减少过思考现象，在缩短推理长度的同时提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR模型存在过思考问题，生成冗余推理步骤但性能无提升。轨迹级长度惩罚方法无法有效缩短推理长度且会降低准确性，过程监督方法资源消耗大且信用分配不准确。

Method: 识别对必要步骤关注度高而对冗余步骤关注度低的特殊注意力头，利用这些注意力头的分数进行步骤级信用分配。采用两种子策略：抑制冗余步骤的同时减少对必要步骤的惩罚。

Result: 在9个基准测试中显著减少推理长度，同时显著提升性能表现。

Conclusion: ATTNPO框架通过利用模型内在注意力信号实现了低开销的过程监督，有效解决了RLVR模型中的过思考问题，在保持准确性的同时缩短了推理过程。
...
### [153] [ViMultiChoice: Toward a Method That Gives Explanation for Multiple-Choice Reading Comprehension in Vietnamese](https://arxiv.org/abs/2602.09961)
*Trung Tien Cao,Lam Minh Thai,Nghia Hieu Nguyen,Duc-Vu Nguyen,Ngan Luu-Thuy Nguyen*

Main category: cs.CL

TL;DR: 提出ViMultiChoice方法，用于越南语阅读理解任务，联合预测答案和生成解释，在ViMMRC 2.0基准和新数据集上达到SotA性能


<details>
  <summary>Details</summary>
Motivation: 现有多选阅读理解模型缺乏解释推理过程的能力，需要开发能同时选择答案并生成解释的模型

Method: 提出ViMultiChoice方法，专门针对越南语阅读理解设计，联合训练选项决策和解释生成任务

Result: 在ViMMRC 2.0基准和新数据集上超越现有基线，达到最先进性能，联合训练显著提升了多选准确性

Conclusion: 联合训练选项决策和解释生成能有效提升多选阅读理解模型的性能，ViMultiChoice为越南语MCRC任务提供了有效解决方案
...
### [154] [A Unified Assessment of the Poverty of the Stimulus Argument for Neural Language Models](https://arxiv.org/abs/2602.09992)
*Xiulin Yang,Arianna Bisazza,Nathan Schneider,Ethan Gotlieb Wilcox*

Main category: cs.CL

TL;DR: 神经语言模型在有限输入下能展现语法泛化能力，挑战了刺激贫乏假说认为需要先天语言约束的观点，但模型的数据效率仍低于儿童，且认知偏置增强未显著改善PoSH基准表现


<details>
  <summary>Details</summary>
Motivation: 检验刺激贫乏假说(PoSH)的核心主张——是否必须依赖先天语言约束才能从有限输入中习得语法泛化能力，通过神经语言模型这一缺乏语言特定约束的计算系统进行验证

Method: 开发POSHBench评估套件，针对英语疑问句形成、移位岛等PoSH核心现象；在1000-5000万单词的发展合理性文本上训练Transformer模型；测试三种认知启发的归纳偏置对模型性能的影响

Result: 模型在没有直接正面证据的情况下对所有现象都显示出泛化迹象；神经模型的数据效率低于儿童且泛化能力较弱；认知偏置提高了整体句法能力但未改善POSHBench表现

Conclusion: 研究挑战了先天句法是实现泛化的唯一途径这一观点，表明类人的数据效率需要超越当前测试的归纳偏置
...
### [155] [ViSpeechFormer: A Phonemic Approach for Vietnamese Automatic Speech Recognition](https://arxiv.org/abs/2602.10003)
*Khoa Anh Nguyen,Long Minh Hoang,Nghia Hieu Nguyen,Luan Thanh Nguyen,Ngan Luu-Thuy Nguyen*

Main category: cs.CL

TL;DR: ViSpeechFormer：基于音素的越南语语音识别框架，利用越南语高度透明的字形-音素对应关系，在公开数据集上表现出色，泛化能力更强且受训练偏差影响更小


<details>
  <summary>Details</summary>
Motivation: 越南语具有语音正字法特征，每个字形最多对应一个音素，这种高度透明的字形-音素关系为基于音素的语音识别提供了独特优势

Method: 提出ViSpeechFormer（越南语音Transformer），首次在越南语ASR中显式建模音素表示，利用越南语的高字形-音素透明度

Result: 在两个公开越南语ASR数据集上表现出强大性能，对词汇表外单词具有更好的泛化能力，且受训练偏差的影响更小

Conclusion: 这种基于音素的范式对于其他具有语音正字法的语言也具有应用前景，为语音识别研究提供了新的方向
...
### [156] [SCORE: Specificity, Context Utilization, Robustness, and Relevance for Reference-Free LLM Evaluation](https://arxiv.org/abs/2602.10017)
*Homaira Huda Shomee,Rochana Chaturvedi,Yangxinyu Xie,Tanwi Mallick*

Main category: cs.CL

TL;DR: 提出了一个多维度、无参考的评估框架，用于评估大语言模型在领域特定问答中的表现，重点关注特异性、鲁棒性、相关性和上下文利用四个维度，并创建了包含1412个专业领域问答对的数据集进行系统评估。


<details>
  <summary>Details</summary>
Motivation: 现有RAG和开放式问答评估框架主要依赖表面相似性、事实一致性和语义相关性，无法有效评估领域敏感决策所需的特定信息，无法满足高风险应用场景的需求。

Method: 开发了四维评估框架：特异性、对改写和语义扰动的鲁棒性、答案相关性、上下文利用；构建了涵盖40个专业角色和7种自然灾害类型的1412个问答对数据集；进行了人工评估以验证模型输出与人类判断的一致性。

Result: 研究表明，没有任何单一指标能够充分捕捉答案质量，证明了在高风险应用场景中部署LLM时需要结构化、多指标评估框架的必要性。人工评估显示了领域特定开放式评估存在固有主观性。

Conclusion: 在高风险的领域特定应用中，需要采用多维度的评估框架来全面评估LLM输出的质量，单一评估指标不足以满足实际应用需求，强调了结构化多指标评估的重要性。
...
### [157] [Decoupled Reasoning with Implicit Fact Tokens (DRIFT): A Dual-Model Framework for Efficient Long-Context Inference](https://arxiv.org/abs/2602.10021)
*Wenxuan Xie,Yujia Wang,Xin Tan,Chaochao Lu,Xia Hu,Xuhong Wang*

Main category: cs.CL

TL;DR: DRIFT是一种新颖的双模型架构，通过轻量级知识模型动态压缩文档块为隐式事实标记，将知识提取与推理过程解耦，显著提升长上下文任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如RAG和知识编辑）受到有限上下文窗口、检索器噪声和灾难性遗忘的限制，需要解决知识数据与推理模式的固有纠缠问题。

Method: 采用双模型架构：轻量级知识模型动态压缩查询相关的文档块为隐式事实标记，然后将这些密集表示投影到推理模型的嵌入空间中。

Result: 大量实验表明，DRIFT在长上下文任务上显著优于同类规模基线模型，在保持推理准确性的同时提高了性能。

Conclusion: DRIFT为扩展LLMs的有效上下文窗口和推理能力提供了可扩展且高效的范式，成功解决了知识整合中的关键挑战。
...
### [158] [MEVER: Multi-Modal and Explainable Claim Verification with Graph-based Evidence Retrieval](https://arxiv.org/abs/2602.10023)
*Delvin Ce Zhang,Suhan Cui,Zhelin Chu,Xianren Zhang,Dongwon Lee*

Main category: cs.CL

TL;DR: 提出一个多模态联合模型，实现证据检索、多模态声明验证和解释生成，特别针对科学图表声明验证任务，并构建了AIChartClaim数据集。


<details>
  <summary>Details</summary>
Motivation: 现有声明验证工作主要关注文本证据推理或忽略可解释性，导致验证结果不准确且缺乏说服力。需要同时处理文本和视觉证据的多模态推理，并提供可解释的验证过程。

Method: 1) 构建双层多模态图进行证据检索，设计图像到文本和文本到图像的跨模态推理；2) 提出token级和证据级融合机制进行多模态声明验证；3) 引入多模态Fusion-in-Decoder方法生成解释；4) 创建AIChartClaim科学数据集。

Result: 实验结果表明该模型在证据检索、声明验证和解释生成方面表现出色，验证了方法的有效性。

Conclusion: 该模型通过联合多模态推理和可解释性生成，有效解决了声明验证中的准确性和可信度问题，为科学领域的多模态声明验证提供了新的解决方案和基准数据集。
...
### [159] [Anagent For Enhancing Scientific Table & Figure Analysis](https://arxiv.org/abs/2602.10081)
*Xuehang Guo,Zhiyong Lu,Tom Hope,Qingyun Wang*

Main category: cs.CL

TL;DR: 提出了AnaBench基准测试和Anagent多智能体框架，用于解决科学图表分析的复杂挑战，通过四个专业智能体协作实现高质量分析，在170个子域评估中显示显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统难以准确解释复杂的多模态科学知识，整合不同来源的证据，并进行基于领域知识的推理。科学图表的结构异质性和长上下文需求构成了根本性障碍。

Method: 开发AnaBench基准测试（63,178个实例，9个科学领域，7个复杂度维度）；提出Anagent多智能体框架：Planner（任务分解）、Expert（工具执行检索）、Solver（信息合成分析）、Critic（五维质量评估迭代优化）；采用监督微调和专业化强化学习的模块化训练策略。

Result: 在170个子域的全面评估中，Anagent实现了显著改进：无训练设置下提升↑13.43%，微调后提升↑42.12%；证明了面向任务的推理和上下文感知问题解决对高质量科学图表分析至关重要。

Conclusion: Anagent框架通过多智能体协作和专业化训练策略，有效解决了科学图表分析的复杂挑战，为AI系统在科学知识解释和推理方面提供了新的解决方案。
...
### [160] [Quantum-Audit: Evaluating the Reasoning Limits of LLMs on Quantum Computing](https://arxiv.org/abs/2602.10092)
*Mohamed Afane,Kayla Laufer,Wenqi Wei,Ying Mao,Junaid Farooq,Ying Wang,Juntao Chen*

Main category: cs.CL

TL;DR: Quantum-Audit是一个包含2700个问题的基准测试，系统评估了语言模型在量子计算概念理解方面的能力，发现顶级模型表现超过人类专家平均水平但存在明显缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估量子代码生成和电路设计，但缺乏对量子计算概念理解能力的系统测量，需要填补这一空白。

Method: 创建包含2700个问题的基准：1000个专家编写问题、1000个从研究论文提取并由专家验证的问题、700个附加问题（350个开放式问题和350个包含错误前提的问题）。评估了26个领先组织的模型。

Result: 人类参与者得分23%-86%，专家平均74%。顶级模型Claude Opus 4.5达到84%准确率，但在专家编写问题上准确率下降12个百分点，安全问题上降至73%。模型经常接受错误前提而非识别，关键推理任务准确率低于66%。

Conclusion: 语言模型在量子计算概念理解方面表现出色但存在局限性，特别是在处理专家编写问题、高级主题和识别错误前提方面需要改进，Quantum-Audit为评估模型量子计算知识提供了有效工具。
